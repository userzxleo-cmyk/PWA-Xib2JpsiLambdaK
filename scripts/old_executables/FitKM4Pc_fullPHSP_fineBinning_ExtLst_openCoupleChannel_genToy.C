...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl; 
  ...
  int iMultiCore = atoi ( argv[9] );
  int is_sig = atoi ( argv[10] );

  TString name_initial_setting = TString(argv[11]);
  name_initial_setting = "func/"+name_initial_setting+".func";

  TString component_label;
  if (is_sig==1) component_label = "sig";
  else if (is_sig==0) component_label = "bkg";
  else {
        std::cout << "is_sig parameter not 0 or 1 ! " << std::endl;
        return 0;
  }
...
//  cosk:cosmu:chi
  ...
  obs1->add(sw);
  TFile *fdata = new TFile("../Fit/sPlot_small.root");
...
//
////open Sigmac D channels for 4440, 4457
//   rg2_Z5.setVal(0.1);
//   rg2_Z5.setRange(-50., 50.);
//   rg2_Z5.setError(0.1);
//   rg2_Z5.setConstant(0);
//   rg2_Z.setVal(0.1);
//   rg2_Z.setRange(-50., 50.);
//   rg2_Z.setError(0.1);
//   rg2_Z.setConstant(0);
//
////   m0_1910.setVal(1.6);
////   m0_1910.setRange(1.45, 2.6);
////   m0_2130.setVal(1.6);
////   m0_2130.setRange(1.45, 2.6);
////   m0_2340.setVal(1.6);
////   m0_2340.setRange(1.45, 2.6);
////   m0_2320.setVal(1.6);
////   m0_2320.setRange(1.45, 2.6);
//
...
//
//  std::cout << "will write to file: " << TString::Format("func/testKM4Pc-fullPHSP-fineBinning_ExtLst_openCoupleChannel-4440%s-4457%s-4312%s-4600%s-"+label_for_output+".func",nameZ5, name, nameZ4,nameZ2).Data() << std::endl;


//  NLL2 = doFit(obs, list, listZ, listX,  data_fit, "hehe", FFR_var, FFB_var,  0, true, errlist);

//  setdlz->writeToFile(TString::Format("func/testKM4Pc-fullPHSP-fineBinning_ExtLst_openCoupleChannel-4440%s-4457%s-4312%s-4600%s-"+label_for_output+".func",nameZ5, name, nameZ4,nameZ2).Data());
//  setdlz->Print("V");
...
//  return 0;
  if(1){
  RooDalitzAmplitude *signew = new RooDalitzAmplitude("signew","", *obs, list, listZ, listX, *FFR_var, *FFB_var, TString::Format("../gen_toys/assign_Lb_mass/output/phsp_withangle_withLbMass_"+component_label+"_%d.root", iMultiCore),*data_fit, true);
//  setdlznew = setdlz;
...
//  signew->genToy("Toy");        
  ...
  signew->genToy(TString::Format("../gen_toys/assign_Lb_mass/output/phsp_withangle_withLbMass_"+component_label+"_%d.root", iMultiCore),TString::Format("testKM4Pc-4440%s-4457%s-4312%s-4600%s-"+component_label+"-%d",nameZ5, name, nameZ4,nameZ2, iMultiCore).Data(), true, true, true, true);
  signew->Delete(); 
  }
  return 0;
  if(1) {
