...
auto parse_args(int argc, char **argv){
  ...
  options
    ...
    ("r,result", "Path to RooFitResult", cxxopts::value<TString>())
...
//  if(!(ZS4==3&&ZP4==1)) {
  ...
  std::cout << "read from file: " << args["result"].as<TString>() << std::endl;
  ...
  for ( auto iter = pars_values->constPars().begin(); iter < pars_values->constPars().end(); iter++ ) {
    ...
    std::cout << "Set the value for: " << varName << std::endl;
  }
  ...
  RooDalitzAmplitude *signew = new RooDalitzAmplitude(
      "signew",
      "",
      *obs,
      list,
  ...
  std::cout << "Calculating Amp^2 weights for file " << args["mc_tuple"].as<TString>() << std::endl;
  sigcopy->storeResonanceWeights(
