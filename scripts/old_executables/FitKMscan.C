...
float mbin[] = {4.025,4.075,4.1,4.125,4.15,4.175,4.2,4.225,4.25,4.275,4.3,4.325,4.35,4.375,4.4,4.425,4.5,4.6,4.65,4.7,4.75,4.8};
//Z 4457
//Z4 4312
//Z5 4440
#if 0
...
//  FloatZPar(L_Z5,false,false,true);
  m0_Z2.setRange(mbin[id]+0.01,mbin[id+1]+0.01);
  m0_Z2.setVal((mbin[id]+mbin[id+1])/2+0.01);
  m0_Z2.setError(0.025);
  g1_Z2.setRange(-2,2);
  g1_Z2.setVal(0.5);
  rg2_Z2.setVal(0); rg2_Z2.setConstant(1);  
  rg3_Z2.setVal(0); rg3_Z2.setConstant(1);    
  rg4_Z2.setVal(0); rg4_Z2.setConstant(1);    
  FloatZPar(L_Z2BW,true);
...
//  signew->genToy("Toy");        
  ...
  signew->genToy("mcsw.root",TString::Format("testKM4PcScan-4440%s-4457%s-4312%s-new%s",nameZ5, name, nameZ4,nameZ2).Data());
  signew->Delete(); 
  }
  if(1) {
