...
int main(int argc, char **argv) {
    ...
    auto parser_args = Parser::parse_args<has_broad_Pc>(
        argc,
        argv,
        "systematic_KMatrixLs",
        "Fits systematic model: Use single-channel K-matrix to model Lz decays");
...
#include "RooTDalitz/ResZKM_alternateCoupling.h"
    ...
    // Randomize masses and widths of new Ls with Gaussian of width 50 MeV
    if (arguments.randomLs) {
        for (RooArgList *list : {L_1910, L_2130, L_2340, L_2320}) {
            const size_t mass_index = list->getSize() - 5;
            auto *mass = dynamic_cast<RooRealVar *>(list->at(mass_index));
            const double mass_delta = 0.05 * Random::get_generator().Gaus();
            Parameter::random_smear(mass, mass_delta);
            const size_t width_index = list->getSize() - 4;
            auto *width = dynamic_cast<RooRealVar *>(list->at(width_index));
            const double width_delta = 0.05 * Random::get_generator().Gaus();
            Parameter::random_smear(width, width_delta);
        }
    }
