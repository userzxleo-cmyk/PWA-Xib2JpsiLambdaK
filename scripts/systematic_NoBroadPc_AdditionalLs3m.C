...
int main(int argc, char **argv) {
    ...
    auto parser_args = Parser::parse_args<has_broad_Pc>(
        argc,
        argv,
        "systematic_NoBroadPc_AdditionalLs3m",
        "Fits systematic model: replace BroadPc with 3/2- Lz state");
...
#include "RooTDalitz/ResZKM_alternateCoupling.h"
    ...
    // Additional L_2400 state with JP = 3/2- to replace broad Pc
    list->Add(L_2400);

    // Ensures that the LS couplings are floating
    for (int i = 0; i < 6; i++) {
        Parameter::FloatPar(L_2400, i);
    }
