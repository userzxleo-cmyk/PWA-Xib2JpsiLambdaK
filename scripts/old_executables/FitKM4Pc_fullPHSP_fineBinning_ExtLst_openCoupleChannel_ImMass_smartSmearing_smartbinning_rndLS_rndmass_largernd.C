...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl; 
  ...
  TString name_initial_setting = TString(argv[9]);
  TString label_for_output = TString(argv[10]);
  label_for_output = "func/"+label_for_output+".func"; 
  name_initial_setting = "func/"+name_initial_setting+".func";

  TString fitresultName = TString(argv[11]);
  fitresultName = "fitresults/"+fitresultName+".root";

  float scale_gJpsip_4457 = atof(argv[12]);
  float widthUpperLimit = atof(argv[14]);
  float widthLowerLimit = atof(argv[13]);
...
//  list->Add(L_2000);  
  ...
  RooDalitzAmplitude *sig = new RooDalitzAmplitude("sig","", *obs, list, listZ, listX, *FFR_var, *FFB_var,  "one.root",*data_fit, *widthUpperLimit_var, *widthLowerLimit_var, *CalFWHM_nbin_var, *CalFWHM_range_var);  
...
//  setdlz->readFromFile(TString::Format("func/testKM4Pc-4440%s-4457%s-4312%s-4600%s.func",nameZ5, name, nameZ4,nameZ2).Data()); 
  ...
  std::cout << "read from file: " << name_initial_setting << std::endl;
  setdlz->Print("V");  
//  a_Bp_Z4.setVal(0);
//  p_Bp_Z4.setVal(0);
//  a_L0_Z4.setVal(0);
...
//  p_L2_Z4.setError(0.1);
  ...
  FloatZPar(L_Z,false,RND,false,RND, RND);
  FloatZPar(L_Z4,false,RND, false, RND, RND); //zero 4312
#ifdef FourPc
  FloatZPar(L_Z2,false,RND, false, RND, RND);
#endif
  FloatZPar(L_ZM1SC1,false);
  FloatZPar(L_ZM1SC2,false);
  FloatZPar(L_ZM1SC3,false);
  FloatZPar(L_ZM1SC4,false);
  FloatZPar(L_Z5,false,RND, false, RND, RND);
  FloatZPar(L_ZM3SC1,false);
  FloatZPar(L_ZM3SC2,false);
  FloatZPar(L_ZM3SC3,false);
  FloatZPar(L_ZM3SC4,false);

  SetLPar(L_1520,6,RND, false, RND);
  SetLPar(L_1600,6,RND, false, RND);
  SetLPar(L_1690,6,RND, false, RND);
  SetLPar(L_1810,6,RND, false, RND);
  SetLPar(L_1820,6,RND, false, RND);
  SetLPar(L_1830,6,RND, false, RND);
  SetLPar(L_1890,6,RND, false, RND);
  SetLPar(L_2100,6,RND, false, RND);
  SetLPar(L_2110,6,RND, false, RND);
  SetLPar(L_2350,6,RND, false, RND);
  SetLPar(L_2585,4,RND, false, RND);
  SetLPar(L_1710,6,RND, false, RND);
  SetLPar(L_2020,6,RND, false, RND);
  SetLPar(L_2050,6,RND, false, RND);


   SetLPar(L_1910, 6, RND, false, RND);
   SetLPar(L_2130, 6, RND, false, RND);
   SetLPar(L_2340, 6, RND, false, RND);
   SetLPar(L_2320, 6, RND, false, RND);
...
//  width_1520.setConstant(0);
  ...
  std::cout << "will write to file: " << label_for_output << std::endl;
...
//  g1_Z.setRange(-10., 10.);
//  float g1_Z_val = g1_Z.getVal();
//  g1_Z.setVal(g1_Z_val*scale_gJpsip_4457);

//rnd the SigmacD couplings, which are not easy to write in Setup.h
  if(!(rg2_Z.isConstant())) rg2_Z.setVal(  rg2_Z.getVal() *  (1+g_rnd->Gaus()*0.2) );
  if(!(rg3_Z.isConstant())) rg3_Z.setVal(  rg3_Z.getVal() *  (1+g_rnd->Gaus()*0.2) );
  if(!(rg4_Z.isConstant())) rg4_Z.setVal(  rg4_Z.getVal() *  (1+g_rnd->Gaus()*0.2) );
  if(!(rg2_Z4.isConstant())) rg2_Z4.setVal(  rg2_Z4.getVal() *  (1+g_rnd->Gaus()*0.2) );
  if(!(rg4_Z4.isConstant())) rg4_Z4.setVal(  rg4_Z4.getVal() *  (1+g_rnd->Gaus()*0.2) );
  if(!(rg2_Z5.isConstant())) rg2_Z5.setVal(  rg2_Z5.getVal() *  (1+g_rnd->Gaus()*0.2) );
  if(!(rg3_Z5.isConstant())) rg3_Z5.setVal(  rg3_Z5.getVal() *  (1+g_rnd->Gaus()*0.2) );
  if(!(rg4_Z5.isConstant())) rg4_Z5.setVal(  rg4_Z5.getVal() *  (1+g_rnd->Gaus()*0.2) );
//rnd the NR components 
   if(!(fsc_ZM1SC1.isConstant())) fsc_ZM1SC1.setVal(  fsc_ZM1SC1.getVal() *  (1+g_rnd->Gaus()*0.2) );
   if(!(fsc_ZM1SC2.isConstant())) fsc_ZM1SC2.setVal(  fsc_ZM1SC2.getVal() *  (1+g_rnd->Gaus()*0.2) );
   if(!(fsc_ZM1SC3.isConstant())) fsc_ZM1SC3.setVal(  fsc_ZM1SC3.getVal() *  (1+g_rnd->Gaus()*0.2) );
   if(!(fsc_ZM1SC4.isConstant())) fsc_ZM1SC4.setVal(  fsc_ZM1SC4.getVal() *  (1+g_rnd->Gaus()*0.2) );
   if(!(fsc_ZM3SC1.isConstant())) fsc_ZM3SC1.setVal(  fsc_ZM3SC1.getVal() *  (1+g_rnd->Gaus()*0.2) );
   if(!(fsc_ZM3SC2.isConstant())) fsc_ZM3SC2.setVal(  fsc_ZM3SC2.getVal() *  (1+g_rnd->Gaus()*0.2) );
   if(!(fsc_ZM3SC3.isConstant())) fsc_ZM3SC3.setVal(  fsc_ZM3SC3.getVal() *  (1+g_rnd->Gaus()*0.2) );
   if(!(fsc_ZM3SC4.isConstant())) fsc_ZM3SC4.setVal(  fsc_ZM3SC4.getVal() *  (1+g_rnd->Gaus()*0.2) );
 


  NLL2 = doFit(obs, list, listZ, listX,  data_fit, fitresultName, FFR_var, FFB_var, widthUpperLimit_var, widthLowerLimit_var, CalFWHM_nbin_var, CalFWHM_range_var, 0, true, errlist, false, false);

  setdlz->writeToFile(label_for_output);
  setdlz->Print("V");
  std::cout << "NLLHalfwFloat " << NLL2 << std::endl;    
//   double ll26 = LL(obs, list, listZ, listX,  data_fit26,  "mcsw_26.root", FFR_var, FFB_var);
//   printf("LLCheck-all-26: %10.3f,  %10.3f\n",NLL2,ll26);
  ...
  return 0;
  if(0){
  RooDalitzAmplitude *signew = new RooDalitzAmplitude("signew","", *obs, list, listZ, listX, *FFR_var, *FFB_var, "mcsw.root",*data_fit, *widthUpperLimit_var, *widthLowerLimit_var, *CalFWHM_nbin_var, *CalFWHM_range_var);
//  setdlznew = setdlz;
...
//  signew->genToy("Toy");        
  ...
  if(1) {
  RooDalitzAmplitude *sigflat = new RooDalitzAmplitude("sigflat","", *obs, list, listZ, listX, *FFR_var, *FFB_var, "MCFlatAcc.root", *data_fit, *widthUpperLimit_var, *widthLowerLimit_var, *CalFWHM_nbin_var, *CalFWHM_range_var);
