...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Submit SigmacstDPole systematic fits with randomised initial parameters."
    )
    parser.add_argument(
        "--func",
        type=str,
    ...
    parser.add_argument(
        "--seed_offset", type=int, default=6000, help="Default offset of random seed."
    )  # 6000
...
def write_sub_file(
    ...
    with open(os.path.join(workdir, sub_name), "w") as file:
        file.write(f"executable = {syracuse_script_dir}/submit_SigmacstDPole.sh\n")
...
def write_submission_file(
    ...
    tag_prefix = f"SigmacstDPole-{narrow_Pc_spin_parity}"
