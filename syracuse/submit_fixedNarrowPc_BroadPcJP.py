...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Submit BroadPcJP systematic fits with randomised initial parameters."
    )
    parser.add_argument(
        "--func",
        type=str,
    ...
    parser.add_argument(
        "--BroadPcJP", type=int, nargs=2, required=True, help="Spin-parity of BroadPc."
    )
...
def get_BroadPc_tag(spin_broad, parity_broad):
    if parity_broad == -1:
        parity_tag = "m"
    elif parity_broad == 1:
        parity_tag = "p"
        ...
        error_string = (
            f"Unrecognised parity for BroadPc: {parity_broad}. Only 1 or -1 allowed!"
        )
        raise ValueError(error_string)

    if spin_broad % 2 != 1:
        error_string = (
            f"Unphysical spin: {spin_broad}. Only odd values of 2*spin is allowed!"
        )
        raise ValueError(error_string)

    tag = f"fixedNarrowPc_BroadPcJP{spin_broad}{parity_tag}"
    return tag
...
def write_sub_file(
    ...
    with open(os.path.join(workdir, sub_name), "w") as file:
        file.write(f"executable = {syracuse_script_dir}/submit_fixedNarrowPc_BroadPcJP.sh\n")
        ...
        file.write("\n")
        file.write(
            f"arguments  = {spin_4440} {spin_4457} {spin_broad} {parity_broad} $(Seed) {tag_prefix}_seed$(Seed) {func_path}\n"
        )
...
def write_submission_file(
    ...
    spin_broad = BroadPcJP[0]
    parity_broad = BroadPcJP[1]
    tag_BroadPc = get_BroadPc_tag(spin_broad, parity_broad)
    tag_prefix = f"{tag_BroadPc}-{narrow_Pc_spin_parity}"
