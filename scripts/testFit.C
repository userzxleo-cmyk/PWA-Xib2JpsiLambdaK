...
auto parse_args(int argc, char **argv){
  ...
  options
    ...
    ("n,nIterations", "Number of iterations to test NLL timing.", cxxopts::value<int>()->default_value("100"))
    ...
    std::cout << "Number of timed iterations : " + args["nIterations"].as<int>() << std::endl;
...
//  if(!(ZS4==3&&ZP4==1)) {
  ...
  FloatZPar(L_Z,false,false);
  FloatZPar(L_Z4,false,false); //zero 4312
#ifdef FourPc
  FloatZPar(L_Z2,false,false);
#endif
  FloatZPar(L_ZM1SC1,false);
  FloatZPar(L_ZM1SC2,false);
  FloatZPar(L_ZM1SC3,false);
  FloatZPar(L_ZM1SC4,false);
  FloatZPar(L_Z5,false,false);
  FloatZPar(L_ZM3SC1,false);
  FloatZPar(L_ZM3SC2,false);
  FloatZPar(L_ZM3SC3,false);
  FloatZPar(L_ZM3SC4,false);

  SetLPar(L_1520,6,false);
  SetLPar(L_1600,6,false);
  SetLPar(L_1690,6,false);
  SetLPar(L_1810,6,false);
  SetLPar(L_1820,6,false);
  SetLPar(L_1830,6,false);
  SetLPar(L_1890,6,false);
  SetLPar(L_2100,6,false);
  SetLPar(L_2110,6,false);
  SetLPar(L_2350,6,false);
  SetLPar(L_2585,4,false);
  SetLPar(L_1710,6,false);
  SetLPar(L_2020,6,false);
  SetLPar(L_2050,6,false);


   SetLPar(L_1910, 6, false);
   SetLPar(L_2130, 6, false);
   SetLPar(L_2340, 6, false);
   SetLPar(L_2320, 6, false);
...
//  m0_Z4.setError(0.0001);
  ...
  setdlz->readFromFile(args["initial"].as<TString>());
  RooDalitzAmplitude *signew = new RooDalitzAmplitude("signew",
                                                      "",
                                                      *obs,
                                                      list,
                                                      listZ,
                                                      ...
                                                      *CalFWHM_nbin_var,
                                                      *CalFWHM_range_var);
  RooDalitzAmplitude *sigcopy = new RooDalitzAmplitude(*signew, "testNLL");

  std::cout << "Testing GPU timing " << std::endl;
  ...
  RooAbsReal *nll0 = signew->createSUM(*One_Data);
  double tmpnll = (nll0->getVal() - NLL_SHIFT) * swf2;

  std::cout << "Value of NLL no constraint: " << std::setprecision(15) << tmpnll << std::endl;

  size_t nIterations = args["nIterations"].as<int>();
  std::vector <size_t> timing;
  timing.reserve(nIterations);
  #ifdef GPU
  ...
  // Probably convert to double and convert to ms first before calculating sum t, sum t^2

  std::vector<double> s_timing(nIterations);
  std::transform(timing.begin(), timing.end(), s_timing.begin(), [](const size_t time) { return 1e-3 * ((double) time); });
  auto sum_time = std::accumulate(s_timing.begin(), s_timing.end(), 0.);
  auto sum_time_square = std::transform_reduce(
    s_timing.begin(),
    s_timing.end(),
    0,
    std::plus{},
    [](const auto time) { return time * time; }
  );

  double average_time = sum_time / nIterations;
  double deviation = sqrt(sum_time_square / nIterations - average_time * average_time);

  std::cout << "Averaged time for " << nIterations << " iterations:" << std::endl;
  std::cout << std::setprecision(10) << "\t( " << average_time << " +/- " << deviation << " ) ms" << std::endl;
