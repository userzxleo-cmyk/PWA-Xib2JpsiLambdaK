...
int main(int argc, char **argv) {
    ...
    Static_Check::Resonances::check_count<16,        // number of Lz,
                                          11,        // number of Pc,
                                          0>();      // number of Zcs
    Static_Check::Systematics::check_flags<false,    // Isospin Sigmac(*)D(*)
                                           false,    // Linear production term in K-matrix
                                           true,     // Self-scattering for Sigmac(*)D(*) channels
                                           false>(); // Use L+1 for lineshape
    ...
    auto parser_args = Parser::parse_args<has_broad_Pc>(
        argc,
        argv,
        "systematic_SelfScatterNRPVector_Remove4Ls",
        "Fits systematic model: (1) (1) K-matrix channels have different non-resonant P-vector "
        "term, (2) Sigmac(*)D(*) channels have non-zero self-scattering terms, (3) Remove 4 new Lz "
        "resonances and (3) no broad Pc");
...
// Include Sigmac(*)D(*) self-scattering terms
//  and non-resonant production vector terms
#include "RooTDalitz/ResZKM_SelfScattering_NonResonantProduction.h"
    ...
    ratioList->add(f_prod_ZM1SC2);
    ratioList->add(f_prod_ZM1SC3);
    ratioList->add(f_prod_ZM1SC4);
    ratioList->add(f_prod_ZM3SC2);
    ratioList->add(f_prod_ZM3SC3);
    ratioList->add(f_prod_ZM3SC4);
