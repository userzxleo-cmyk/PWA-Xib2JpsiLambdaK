...
{
    nvtxRangePush("config before fit");
    TDatime BeginTime;
...
#include "RooTDalitz/ResZKM.h"
    ...
    printf("======================================second fit====================================\n");
    nvtxRangePush("second fit");
    NLL2 = doFit(obs, list, listZ, data_fit, 0, true, errlist);
    nvtxRangePop();
    setdlz->writeToFile(TString::Format("func/testKMNew3-4440%s-4457%s-4312%s.func", nameZ5, name, nameZ4).Data());
    setdlz->Print("V");
    std::cout << "NLLHalfwFloat " << NLL2 << std::endl;

    printf("======================================26 LL calculation!====================================\n");
    ...
    if (1)
    {
        ...
        signew->genToy("mcsw.root", TString::Format("testKMNew3-4440%s-4457%s-4312%s", nameZ5, name, nameZ4).Data());
        signew->Delete();
    }
    if (1)
    {
        ...
        printf("Total  %6.2f\n", sum * 100.0);
        nres += listZ->GetSize();
        TString lwvname[] = {"L_S01", "L_P01", "L_P03", "L_D03", "L_D05", "L_F05", "L_F07", "L_G07", "L_G09", "L_H09", "Z_S01", "Z_P01", "Z_S03", "Z_P03", "Z_P05", "Z_D05"};
