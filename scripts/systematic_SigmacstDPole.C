...
int main(int argc, char **argv) {
    ...
    auto parser_args = Parser::parse_args<has_broad_Pc>(
        argc,
        argv,
        "systematic_SigmacstDPole",
        "Fits systematic model: Additional Jpsip pole at near the Sigmac*D threshold");
...
#include "RooTDalitz/ResZKM_alternateCoupling.h"
    ...
    ratioList->add(rg1_Z6);
    ratioList->add(rg2_Z6);
    ratioList->add(rg3_Z6);

    for (int i = 0; i < 4; i++) {
        Parameter::FloatPar(L_Z6, i);
    }
