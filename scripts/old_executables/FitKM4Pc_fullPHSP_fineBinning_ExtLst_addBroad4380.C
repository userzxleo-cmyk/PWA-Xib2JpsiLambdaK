...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl; 
  ...
  TString label_for_output = TString(argv[12]);
  ...
  ZS3 = atoi( argv[9] );
  ZP3 = atoi( argv[10] );
  std::cout << "Fit broad 4380 J P" << ZS3 << " " << ZP3 << std::endl;
...
//   m0_2320.setRange(1.45, 2.6);
  ...
  std::cout << "will write to file: " << TString::Format("func/testKM4Pc-fullPHSP-fineBinning-ExtLst-addBroad4380-4440%s-4457%s-4312%s-4600%s-4380%s-"+label_for_output+".func",nameZ5, name, nameZ4,nameZ2,nameZ3).Data() << std::endl;


  NLL2 = doFit(obs, list, listZ, listX,  data_fit, FFR_var, FFB_var, 0, true, errlist);


  setdlz->writeToFile(TString::Format("func/testKM4Pc-fullPHSP-fineBinning-ExtLst-addBroad4380-4440%s-4457%s-4312%s-4600%s-4380%s-"+label_for_output+".func",nameZ5, name, nameZ4,nameZ2, nameZ3).Data());
  return 0;
  setdlz->Print("V");
