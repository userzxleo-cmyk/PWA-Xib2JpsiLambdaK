...
//  rg4_Z4.setConstant(0);   
  ...
  FloatZPar(L_Z2);
  FloatZPar(L_Z5);
  FloatZPar(L_Zm3_1, true);
  FloatZPar(L_Zm3_2, true);
  FloatZPar(L_Zm1_1, true);
  FloatZPar(L_Zm1_2, true);

  NLL2 = doFit(obs, list, listZ, listX,  data_fit, 0, true, errlist);   
  setdlz->writeToFile(TString::Format("func/testKM8Pc-4440%s-4457%s-4312%s-4600%s.func",nameZ5, name, nameZ4,nameZ2).Data());
  setdlz->Print("V");
...
//  signew->genToy("Toy");        
  ...
  signew->genToy("mcsw.root",TString::Format("testKM8Pc-4440%s-4457%s-4312%s-4600%s",nameZ5, name, nameZ4,nameZ2).Data());
  signew->Delete(); 
  }
  if(1) {
