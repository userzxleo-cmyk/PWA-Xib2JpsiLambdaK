...
//  cosk:cosmu:chi
  ...
  obs1->add(sw);
  TFile *fdata = new TFile("../Fit_small/sPlot.root");
...
//   printf("LLCheck-all-26: %10.3f,  %10.3f\n",NLL2,ll26);
  ...
  if(1){
  RooDalitzAmplitude *signew = new RooDalitzAmplitude("signew","", *obs, list, listZ, listX, *FFR_var, *FFB_var, "../Fit_small/mcsw.root",*data_fit);
//  setdlznew = setdlz;
  std::cout << "Generate Toy ...." << std::endl;

  signew->genToy("../Fit_small/mcsw.root",name_initial_setting+"debug");
  signew->Delete(); 
  }
  return 0;
  if(1) {
