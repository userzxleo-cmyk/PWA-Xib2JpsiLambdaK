...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl; 
  ...
  TString name_initial_setting = TString(argv[10]);

  TString round = TString(argv[11]);

  TString label_for_output = "func/"+name_initial_setting+TString::Format("_rndfit_seed%d", seed)+"_"+round+".func";
