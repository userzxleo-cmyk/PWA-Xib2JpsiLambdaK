...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl; 
  ...
  RooRealVar *FFB_var = new RooRealVar("FFB_var","FFB_var",3.5f, 1.f, 5.f);
  RooRealVar *FFR_var = new RooRealVar("FFR_var","FFR_var",3.5f, 1.f, 5.f);
...
//  p_L2_Z4.setError(0.1);
  ...
  std::cout << "will write to file: " << TString::Format("func/testKM4Pc-RandomR-4440%s-4457%s-4312%s-4600%s-"+label_for_output+".func",nameZ5, name, nameZ4,nameZ2).Data() << std::endl;
  NLL2 = doFit(obs, list, listZ, listX,  data_fit, FFR_var, FFB_var, 0, true, errlist);

  setdlz->writeToFile(TString::Format("func/testKM4Pc-RandomR-4440%s-4457%s-4312%s-4600%s-"+label_for_output+".func",nameZ5, name, nameZ4,nameZ2).Data());
  setdlz->Print("V");
