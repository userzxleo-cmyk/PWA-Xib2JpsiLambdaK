...
auto parse_args(int argc, char **argv){
  ...
  options
    ...
    ("i,initial", "Path to initial parameter func", cxxopts::value<TString>())
    ("f,final", "Path to dump final (post-fit) parameter func", cxxopts::value<TString>())
    ("r,result", "Path to store RooFitResult", cxxopts::value<TString>())
    ("w,data_weights", "Path to store per-event amplitude weights for data sample", cxxopts::value<TString>())
    ("e,mc_weights", "Path to store per-event amplitude weights for mc (normalisation) sample", cxxopts::value<TString>())
    ...
    ("random", "Whether to randomise Ls initial parameters", cxxopts::value<bool>()->default_value("false"))
    ("randomPc", "Whether to randomise Pc initial parameters", cxxopts::value<bool>()->default_value("false"))
    ("seed", "Seed for random number generator", cxxopts::value<unsigned int>()->default_value("1234"))
    ...
    std::cout << "Will write data per-event amplitude^2 to : " + args["data_weights"].as<TString>() << std::endl;
    std::cout << "Will write normalisation per-event amplitude^2 to : " + args["mc_weights"].as<TString>() << std::endl;
    std::cout << std::endl;

    auto printBool = [](bool boolean) { std::string statement = boolean ? "True" : "False"; return statement; };

    std::cout << "Random   : " << printBool(args["random"].as<bool>()) << std::endl;
    ...
    if (args["random"].as<bool>() || args["randomPc"].as<bool>()){
      std::cout << "Random seed to randomise initial parameters: " << args["seed"].as<unsigned int>() << std::endl;
      std::cout << std::endl;
    }
...
//  list->Add(L_2000);
  ...
  ratioList->add(rg4_Z4);
  ratioList->add(rg2_Z5);
...
//  if(!(ZS4==3&&ZP4==1)) {
  ...
  double NLL2;
  setdlz->readFromFile(args["initial"].as<TString>());
  setdlz->Print("V");

  double a0=a_p1p1_1600.getVal();
  double p0=p_p1p1_1600.getVal();

  bool random = args["random"].as<bool>();
  bool randomPc = args["randomPc"].as<bool>();
  if (random || randomPc) {
    g_rnd = new TRandom2(args["seed"].as<unsigned int>());
  }

  FloatZPar(L_Z,false,randomPc);
  FloatZPar(L_Z4,false,randomPc); //zero 4312
#ifdef FourPc
  FloatZPar(L_Z2,false,randomPc);
#endif
  FloatZPar(L_ZM1SC1,false);
  FloatZPar(L_ZM1SC2,false);
  FloatZPar(L_ZM1SC3,false);
  FloatZPar(L_ZM1SC4,false);
  FloatZPar(L_Z5,false,randomPc);
  FloatZPar(L_ZM3SC1,false);
  FloatZPar(L_ZM3SC2,false);
  FloatZPar(L_ZM3SC3,false);
  FloatZPar(L_ZM3SC4,false);

  SetLPar(L_1520,6,random);
  SetLPar(L_1600,6,random);
  SetLPar(L_1690,6,random);
  SetLPar(L_1810,6,random);
  SetLPar(L_1820,6,random);
  SetLPar(L_1830,6,random);
  SetLPar(L_1890,6,random);
  SetLPar(L_2100,6,random);
  SetLPar(L_2110,6,random);
  SetLPar(L_2350,6,random);
  SetLPar(L_2585,4,random);
  SetLPar(L_1710,6,random);
  SetLPar(L_2020,6,random);
  SetLPar(L_2050,6,random);


   SetLPar(L_1910, 6, random);
   SetLPar(L_2130, 6, random);
   SetLPar(L_2340, 6, random);
   SetLPar(L_2320, 6, random);

  // Set the uncertainty to be a minimum of 0.05% of value
  for ( auto var = setdlz->begin(); var < setdlz->end(); var++ ) {
...
//  g1_Z.setVal(g1_Z_val*scale_gJpsip_4457);
  ...
  NLL2 = doFit(obs,
               ...
               args["result"].as<TString>(),
               args["data_weights"].as<TString>(),
               args["mc_weights"].as<TString>(),
               FFR_var,
               FFB_var,
               widthUpperLimit_var,
               widthLowerLimit_var,
  ...
  setdlz->writeToFile(args["final"].as<TString>());
  setdlz->Print("V");
  std::cout << "NLLHalfwFloat " << NLL2 << std::endl;
  std::cout << "Finished MIGRAD" << std::endl;
