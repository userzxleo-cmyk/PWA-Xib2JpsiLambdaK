...
//     if(i%1000==0) std::cout << "i " << i << std::endl;
  ...
  RooDataSet *datars2 = (RooDataSet*)datars->reduce("(mjpsip>4.297&&mjpsip<4.327)");
...
//     if(i%1000==0) std::cout << "i " << i << std::endl;
  ...
  datars2->merge(IND2); 
  RooDataSet * data_fit4312 = new  RooDataSet(TString(datars2->GetName())+TString("new26"),datars2->GetTitle(),datars2,*datars2->get(),0,"nsig_sw");
...
//  if(!(ZS4==3&&ZP4==1)) {
  ...
  double NLL2=0;// = doFit(obs, list, listZ,  data_fit, 0, false, errlist);   
...
//  setdlz->readFromFile(TString::Format("func/testKM4Pc-4440%s-4457%s-4312%s-4600%s.func",nameZ5, name, nameZ4,nameZ2).Data()); 
  ...
  std::cout << "NLLHalfwFloat " << NLL2 << std::endl;    
   double ll4312 = LL(obs, list, listZ, listX,  data_fit4312, "mcsw_4312.root", errlist);
   double llfull = LL(obs, list, listZ, listX,  data_fit, "mcsw.root", errlist);
   printf("LLCheck-all-full: %10.3f,  %10.3f\n",NLL2,llfull);
   printf("LLCheck-all-4312: %10.3f,  %10.3f\n",NLL2,ll4312);
