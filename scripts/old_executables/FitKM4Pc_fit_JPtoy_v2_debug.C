...
//  name_toyfile = "../gen_toys/fit_for_sw/output/"+name_toyfile;
//  output_func_name = "func/" + output_func_name;
...
//  std::cout << "name of output func: " << output_func_name << std::endl;
  ...
  RooRealVar *FFB_var = new RooRealVar("FFB_var","FFB_var",3.f, 1.f, 5.f);
  RooRealVar *FFR_var = new RooRealVar("FFR_var","FFR_var",3.f, 1.f, 5.f);
...
//  TFile *fdata = new TFile("sPlot.root");
  TFile *fdata = new TFile("/nishome/wangmz/workspace/Lb2JpsipK/lb2jpsipk_run12_aman/for_Pc4312JP/gen_toys/test/sPlot.root");
...
//  if(!(ZS4==3&&ZP4==1)) {
  ...
  setdlz->readFromFile(name_initial_setting.Data());  
  std::cout << "read from file: " << name_initial_setting.Data() << std::endl;
//  std::cout << "will write to file: " << output_func_name << std::endl;
  ...
  FloatZPar(L_Z4,true,RND); //zero 4312
#ifdef FourPc
  ...
  if((ZS4==3&&ZP4==-1)) {
    ...
    rg4_Z4.setVal(0.1); rg4_Z4.setError(0.1); rg4_Z4.setConstant(0);
  }
...
//  rg4_Z4.setConstant(0);    
  NLL2 = doFit(obs, list, listZ, listX,  data_fit, FFR_var, FFB_var, 0, true, errlist);  
  //NLL2 = doFit(obs, list, listZ, listX,  data_fit, FFR_var, FFB_var, 0, true, errlist, true);  
  setdlz->Print("V");
...
//  std::cout << "process is done" << std::endl;   
  if(0){
  RooDalitzAmplitude *signew = new RooDalitzAmplitude("signew","", *obs, list, listZ, listX, *FFR_var, *FFB_var, "mcsw.root",*data_fit, true);
//  setdlznew = setdlz;
