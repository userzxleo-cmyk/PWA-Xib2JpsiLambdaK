...
//  int seed = atoi( argv[11] );
  ...
  TString label_for_output = name_initial_setting;
  name_initial_setting = "func/"+name_initial_setting+".func";
  TString outputfunc = "func/"+label_for_output+"-ReRunFail.func";
...
//  }
  ...
  std::cout << "will write to file: " << outputfunc << std::endl;
  
  NLL2 = doFit(obs, list, listZ, listX,  data_fit, FFR_var, FFB_var, 0, true, errlist);  
  
  setdlz->writeToFile(outputfunc);
  setdlz->Print("V");
