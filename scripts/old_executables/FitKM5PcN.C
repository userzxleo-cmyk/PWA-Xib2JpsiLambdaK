...
//  list->Add(L_2000);  
  ...
  char nameZ2[10];
  if(ZP5>0) {
   sprintf(nameZ2,"p%i",ZS5);
  } else {
   sprintf(nameZ2,"m%i",ZS5);  
  }    
...
//  setdlz->readFromFile("test9.func");
  setdlz->readFromFile("logsav/testKM5Pc3-4440m3-4457m1-4312m1-4500m3.func");
...
//  FloatZPar(L_Z5,false,false,true);
  ...
  if(!(ZS5==3&&ZP5==-1)) {
    FloatZPar(L_Z5,true); //4440
     g1_Z5.setConstant(1);
     m0_Z5.setConstant(1);
     rg2_Z5.setConstant(1);    
     rg3_Z5.setConstant(1);    
     rg4_Z5.setConstant(1);    
  }
  #endif
...
//  m0_Z5.setVal(4.440);//3/2-
  ...
  g1_Z.setRange(-0.3,0.3);
  g1_Z5.setRange(-0.3,0.3);
  
  
  NLL2 = doFit(obs, list, listZ,  data_fit, 0, true, errlist);   
    
  setdlz->writeToFile(TString::Format("func/testKM5Pc3-4440%s-4457%s-4312%s-4500%s.func",nameZ5, name, nameZ4, nameZ2).Data());
  setdlz->Print("V");
...
//  signew->genToy("Toy");        
  ...
  signew->genToy("mcsw.root",TString::Format("testKM5Pc3-4440%s-4457%s-4312%s-4500%s",nameZ5, name, nameZ4, nameZ2).Data());
  signew->Delete(); 
  }
  if(1) {
