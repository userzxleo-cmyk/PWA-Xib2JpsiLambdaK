...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl;
  ...
  TString initial_roofitresult = TString(argv[9]);
  TString final_func = TString(argv[10]);
  TString final_roofitresult = TString(argv[11]);
  TString toy_data = TString(argv[12]);
  TString toy_mcsw = TString(argv[13]);

  std::cout << "ToyFit : Fit dataset <" << toy_data << ">." << std::endl;
  std::cout << "ToyFit : Normalisation dataset <" << toy_mcsw << ">." << std::endl;

  float scale_gJpsip_4457 = atof(argv[14]);
  float widthUpperLimit = atof(argv[15]);
  float widthLowerLimit = atof(argv[16]);
...
//  cosk:cosmu:chi
  ...
  obs1->add(sw);
  TFile *fdata = new TFile(toy_data);
...
#include "RooTDalitz/ResX.h"
  ...
  TString errlist = "err/PRC_softKMatrixLimit.err";

  std::cout << "ToyFit : Will use initial parameters from <" << initial_roofitresult << ">." << std::endl;
  TFile roofitresult_file(initial_roofitresult, "READ");
  ...
  g_rnd = new TRandom2(1234);
  if ( RND and g_rnd == nullptr){
    std::cout << "Initialize a random number generator before using it!" << std::endl;
    assert(0);
  }
  ...
  // Set the uncertainty to be a minimum of 0.5% of value
  for ( auto var = setdlz->begin(); var < setdlz->end(); var++ ) {
    auto * parameter = dynamic_cast<RooRealVar*>(*var);
    if ( parameter->isConstant() ) continue;
    if ( parameter->getError() < std::abs( 0.005 * parameter->getVal() ) ) {
      parameter->setError( 0.005 * std::abs( parameter->getVal() ) );
    }

    // if ( parameter->hasMin() && ( parameter->getVal() - parameter->getError() <= parameter->getMin() ) ) {
    //   parameter->setVal( parameter->getMin() + 1.1 * parameter->getError() );
    // }

    // if ( parameter->hasMax() && ( parameter->getVal() + parameter->getError() >= parameter->getMax() ) ) {
  ...
  std::cout << "ToyFit : Will save output func to <" << final_func << ">." << std::endl;
  std::cout << "ToyFit : Will save RooFitResult to <" << final_roofitresult << ">." << std::endl;

  NLL2 = doFit(obs, list, listZ, listX,  data_fit, final_roofitresult, FFR_var, FFB_var, widthUpperLimit_var, widthLowerLimit_var, CalFWHM_nbin_var, CalFWHM_range_var, 0, true, errlist, false, false);

  setdlz->writeToFile(final_func);
  setdlz->Print("V");
  std::cout << "NLLHalfwFloat " << NLL2 << std::endl;

  std::cout << "Finished first migrad" << std::endl;
