...
//  int seed = atoi( argv[11] );
  ...
  TString outputname = TString(argv[10]);
  name_initial_setting = "func/"+name_initial_setting+".func";
  int iMultiCore = atoi ( argv[11] );
...
//  if(!(ZS4==3&&ZP4==1)) {
  ...
  //setdlz->readFromFile(TString::Format("func/for_gen_toy/testKM4Pc-4440%s-4457%s-4312%s-4600%s.func",nameZ5, name, nameZ4,nameZ2).Data());  
...
//  setdlz->writeToFile(TString::Format("func/testKM4Pc-4440%s-4457%s-4312%s-4600%s.func",nameZ5, name, nameZ4,nameZ2).Data());
//  setdlz->Print("V");
...
//  signew->genToy("Toy");        
  ...
  signew->genToy(TString::Format("/nishome/wangmz/workspace/Lb2JpsipK/lb2jpsipk_run12_aman/tuple/phsp_mc/output/phsp_withangle_%d.root",iMultiCore),outputname+"_"+TString::Format("%d",iMultiCore));
  signew->Delete(); 
  }
  if(0) {
