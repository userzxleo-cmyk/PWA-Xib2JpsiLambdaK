...
// MCDATA(mcdata)
{
	...
	for (int i = 0; i < ncached; i++)
        ...
        assert(nres <= MYRN + MYZN + MYXN);

	double costheta, costheta1, costheta2, phi1, phi2;
	double mkp, sw, mjpsip, mjpsik;
	double phiZ;
	double phiMu;
	double phiPsi;
	double alphaMu;
	...
	for (int i = 0; i < smcnum; ++i)
	{
		...
		filldlz((*xdlz), mkp, costheta, costheta1, costheta2, phi1, phi2,
				costhetaB, costhetaZ, costhetaPsi, phiZ, phiPsi, phiMu, costhetap, alphaMu,
                                mjpsip,
                                X_cosTheta_Lb, X_cosTheta_X, X_cosTheta_Jpsi, X_phiX, X_phiJpsi, 
                                X_phiMu, X_cosTheta_p, X_alpha_Mu, mjpsik);
	...
	for (int imc = 0; imc < smcnum; imc++)
	{
		...
		filldlz((*xdlz), mkp, costheta, costheta1, costheta2, phi1, phi2,
				costhetaB, costhetaZ, costhetaPsi, phiZ, phiPsi, phiMu, costhetap, alphaMu,
                                mjpsip, X_cosTheta_Lb, X_cosTheta_X, X_cosTheta_Jpsi, X_phiX, 
                                X_phiJpsi, X_phiMu, X_cosTheta_p, X_alpha_Mu, mjpsik,
				sw);
		sumW += sw;
		//    float fx(0);
		PDLZ->push_back(xdlz);
...
RooDalitzAmplitude::RooDalitzAmplitude(const RooDalitzAmplitude &other, const char *name)
	: RooAbsPdf(other, name),
	  ...
	  _resListZ(other._resListZ),
          _resListX(other._resListX),
	  resParams("resParams", this, other.resParams),
	  _Nmc(other._Nmc), _Nda(other._Nda),
	  _Nda_SMR1(other._Nda_SMR1),
	  CG1(other.CG1),
	  CG3(other.CG3),
	  CG5(other.CG5),
	  CG7(other.CG7),
	  ...
	  CG63(other.CG63),
          ...
          CG22(other.CG22),
          CG42(other.CG42),
          _FFR("_FFR",this,other._FFR),
          _FFB("_FFB",this,other._FFB),
	  AA(other.AA),
	  AAZ(other.AAZ),
	  ABZ(other.ABZ),
	  amp(other.amp),
	  ...
	  _vamp(other._vamp),
	  _paraSpline(other._paraSpline),
	  _galpha(other._galpha), _fscat(other._fscat),
	  _sa3(other._sa3), _sp3(other._sp3),
          NR(other.NR), NZ(other.NZ), NX(other.NX),
	  sumW(other.sumW),
	  m_lb(other.m_lb), m_k(other.m_k),
	  m_p(other.m_p), m_jpsi(other.m_jpsi),
	  m_chic1(other.m_chic1),
	  _mynll(other._mynll),
	  _sw(other._sw),
	  sumW_data(other.sumW_data)
{
	int n = PDLZ->size();
	...
	// smcdata->Add(MCDATA);
	//   std::cout << "_galpha size " <<  _galpha[0].size() << " " << _galpha[1].size() << std::endl;
	...
	//int array_num = sizeof(cu_Jpsihh_dlz) / sizeof(float);
        int array_num = 111;//mkp, mjpsip, w, L* chain ARe, AIm, DL
	...
	//array_num = (int(array_num / 8) + 1) * 8; //to force to be 32(=8*sizeof(float)) * N
        std::cout << "size of ... " << array_num << " , " << array_num1 << " ,  " << array_num2 << " ,  " << array_num3 << " ,  " << array_num4 << std::endl;
	...
	for (int i = 0; i < DEVICE_NUM; i++)
	{
		...
		for (int it = Ns[i]; it < Ns[i + 1]; it++)
		{
			...
			h_float_pp[i][it_h_float_pp] = pp->mkp;
			...
			{ //delmu
				...
				{ //hel0
					for (int m = 0; m < 6; ++m)
					{
						h_float_pp[i][it_h_float_pp + (3 + (ii * 6 * 2 + j * 6 + m) * 2) * N_size] = pp->ARe[ii][j][m];
						h_float_pp[i][it_h_float_pp + (3 + (ii * 6 * 2 + j * 6 + m) * 2 + 1) * N_size] = pp->AIm[ii][j][m];
					}
				}
			}
			...
			for (int ii = 0; ii < 5; ++ii)
			{
				for (int k = 0; k < 2; ++k)
				{
					for (int m = 0; m < 6; ++m)
					{
						...
						h_float_pp[i][it_h_float_pp + (51 + ii * 6 * 2 + k * 6 + m) * N_size] = pp->DL[ii][k][m];
					}
				}
			}
			it_h_float_pp++;
		...
		for (int it = Ns[i]; it < Ns[i + 1]; it++)
		{
			...
			{ //delmu
				...
				{ //hel0
					for (int m = 0; m < 12; ++m)
					{
						h_float_pp1[i][it_h_float_pp + ((ii * 12 * 2 + j * 12 + m) * 2) * N_size] = pp->AZRe[ii][j][m];
						h_float_pp1[i][it_h_float_pp + ((ii * 12 * 2 + j * 12 + m) * 2 + 1) * N_size] = pp->AZIm[ii][j][m];
					}
				}
			}
			it_h_float_pp++;
			...
			for (int ii = 0; ii < 4; ++ii)
			{
				for (int k = 0; k < 2; ++k)
				{
					for (int m = 0; m < 12; ++m)
					{
						...
						h_float_pp2[i][it_h_float_pp + (ii * 12 * 2 + k * 12 + m) * N_size] = pp->DZ[ii][k][m];
					}
				}
			}
			it_h_float_pp++;
                        ...
                        for (int it = Ns[i]; it < Ns[i + 1]; it++)
                        {
                                ...
                                { 
                                        ...
                                        { 
                                                for (int m = 0; m < 12; ++m)
                                                {       
                                                        h_float_pp3[i][it_h_float_pp + (1 + (ii * 12 * 2 + j * 12 + m) * 2) * N_size] = pp->AXRe[ii][j][m];                                   
                                                        h_float_pp3[i][it_h_float_pp + (1 + (ii * 12 * 2 + j * 12 + m) * 2 + 1) * N_size] = pp->AXIm[ii][j][m];                       
                                                }
                                        }
                                }
                                it_h_float_pp++;
                                ...
                                {       
                                        ...
                                        {       
                                                ...
                                                {       
                                                        ...
                                                        h_float_pp4[i][it_h_float_pp + (ii * 12 * 2 + k * 12 + m) * N_size] = pp->DX[ii][k][m];                                   
                                                }
                                        }
                                }
                                it_h_float_pp++;
...
void RooDalitzAmplitude::genToy(const TString mcdata, const char *name, bool verbose, bool smear, bool allvar, bool fortoystudy) const
{
	...
	smcdata->SetBranchAddress("mass", &mass);
        if (fortoystudy) smcdata->SetBranchAddress("eff", &eff);
	smcdata->SetBranchAddress("mkp", &mkp);
	smcdata->SetBranchAddress("mjpsip", &mjpsip);
	smcdata->SetBranchAddress("cosTheta_Lb", &costheta);
	smcdata->SetBranchAddress("cosTheta_L", &costheta1);
	...
	double w;
        double wl[MYRN + 1], wz[MYZN + 1], wx[MYXN + 1];

	if (fortoystudy) tree->Branch("eff", &eff, "eff/D");
	...
	TString Lstname[] = {"1405", "1520", "1600", "1670", "1690", "1800", "1810", "1820", "1830", "1890", "2100", "2110", "2350", "2585", "1710", "2000", "2020", "2050"};
	int nLst[] = {1405, 1520, 1600, 1670, 1690, 1800, 1810, 1820, 1830, 1890, 2100, 2110, 2350, 2585, 1710, 2000, 2020, 2050};
	double wlwv[20];
	double wZwv[20];
	double wLst[40];
	for (int ii = 0; ii < NZ; ++ii)
	{
        ...
        for(int ii=0; ii<NX; ++ii)
                tree->Branch(TString("w")+TString((_resListX->At(ii))->GetName()), &wx[ii+1], TString("w")+TString((_resListX->At(ii))->GetName())+TString("/D"));
	for (int ii = 0; ii < 6; ++ii)
		tree->Branch(TString("wPWZ") + Zwvname[ii], &wZwv[ii], TString("wPWZ") + Zwvname[ii] + TString("/D"));
	if (verbose)
	{

		for (int ii = 0; ii < 10; ++ii)
			tree->Branch(TString("wPW") + lwvname[ii], &wlwv[ii], TString("wPW") + lwvname[ii] + TString("/D"));
		//  for(int ii=0; ii<NR; ++ii)
		//      tree->Branch(TString("wLst")+Lstname[ii], &wLst[ii], TString("wLst")+Lstname[ii]+TString("/D"));
	...
	for (int imc = 0; imc < smcnum; imc++)
	{
		...
		//    w = val;
		//calculcate each wi
                wl[0] = GetAmp2sForInt_SMR(p, -1, 1) * sweight;
                wz[0] = GetAmp2sForInt_SMR(p, -1, 2) * sweight;
                wx[0] = GetAmp2sForInt_SMR(p,-1,10) * sweight;
		...
		for (int ii = 0; ii < NZ; ++ii)
		{
			if (!(TString((_resListZ->At(ii))->GetName())).Contains("SC"))
                ...
                for(int ii=0; ii<NX; ++ii)
                        wx[ii+1] = GetAmp2sForInt_SMR(p,ii+NR+NZ) * sweight;
		for (int ii = 0; ii < 6; ++ii)
                        wZwv[ii] = GetAmp2sForInt_SMR(p, nZwv[ii], 5) * sweight;
		if (verbose)
		{
			for (int ii = 0; ii < 10; ++ii)
                                wlwv[ii] = GetAmp2sForInt_SMR(p, nlwv[ii], 3) * sweight;
...
void RooDalitzAmplitude::ModelIndependentWave3(float *fPoly, float x, float &Rea, float &Ima) const
{
	float sumre(0.0);
	float sumim(0.0);
	Rea = 0;
	...
	float fXmax = m_lb - m_jpsi + 0.002; //2.185;//4.416;//5.03;//2.53;//((RooAbsReal&)res24[4]).getVal();
...
void RooDalitzAmplitude::ModelIndependentWave(float *fPoly, float x, float &Rea, float &Ima) const
{
	...
	float fXmax = 4.6;					//m_lb-m_k+0.002;//2.185;//4.416;//5.03;//2.53;//((RooAbsReal&)res24[4]).getVal();
...
Double_t RooDalitzAmplitude::IntegralMethod2() const
{
	...
	for (int i = 0; i < iEnd; i++)
	{
		...
		_fx[i] = val;
		//    std::cout << "mkp, pdf " << p.mkp << " " << val << std::endl;
		if (i >= _Nda)
			inte += val * sweight;
		else
			wmz_data_inte += val;
	...
	nvtxRangePush("host_store_fx");
        inte = host_store_fx(d_float_pp, d_float_pp1, d_float_pp2, d_float_pp3, d_float_pp4, d_float_sw, h_paraList, _paraList.size(), _vamp.size(), _paraSpline.size(), h_fx, iEnd, 0, _Nda, _Nda_SMR1 - 1, NR, NZ, NX, _mynll, sumW, sumW_data, _FFR, _FFB);
	nvtxRangePop();
	...
	//inte = inte / sumW;
	nvtxRangePop();
	//  std::cout << "wmz MC inte: " << inte << std::endl;
	//  std::cout << "wmz data inte: " << wmz_data_inte << std::endl;
	//return inte;
	//zhihong
	return 1;
}
			...
			for (int j = 0; j < 2; ++j)
			{
				for (int k = 0; k < 2; ++k)
				{
					for (int ir1 = 0; ir1 < NR; ++ir1)
					{
						for (int m1 = 0; m1 < 6; ++m1)
						{
							TComplex tp1 = TComplex((double)p.ARe[ir1][i][j][k][m1], -(double)p.AIm[ir1][i][j][k][m1]);
							for (int ir2 = 0; ir2 < NR; ++ir2)
							{
								for (int m2 = 0; m2 < 6; ++m2)
								{
									TComplex tp2 = TComplex((double)p.ARe[ir2][i][j][k][m2], (double)p.AIm[ir2][i][j][k][m2]);
									tctmp = tp1 * tp2;
			...
			for (int j = 0; j < 2; ++j)
			{
				for (int k = 0; k < 2; ++k)
				{
					for (int ir1 = 0; ir1 < NZ; ++ir1)
					{
						for (int m1 = 0; m1 < 12; ++m1)
						{
							TComplex tp1 = TComplex((double)p.AZRe[ir1][i][j][k][m1], -(double)p.AZIm[ir1][i][j][k][m1]);
							for (int ir2 = 0; ir2 < NZ; ++ir2)
							{
								for (int m2 = 0; m2 < 12; ++m2)
								{
									TComplex tp2 = TComplex((double)p.AZRe[ir2][i][j][k][m2], (double)p.AZIm[ir2][i][j][k][m2]);
									tctmp = tp1 * tp2;
...
#endif
	/*  if(code!=7) {
//    for(int i=0; i<NR+NZ; ++i) _paraList[i*nset+5] = 0;
  }*/
	if (code == 7)
...
TComplex RooDalitzAmplitude::MEangPart(float hel0, int delhelmu,
									   float hel1, int hel2, float phi1, float phi2,
									   float theta, float theta2) const
{
	// float phi1 = dlz.phi1;
	//float phi2 = dlz.phi2;
	...
	return TComplex(1, partphi, 1) * (double)dfunhalf(theta, (int)(hel0 * 2.), (int)((hel1 - hel2) * 2.)) * (double)dfunone(theta2, hel2, delhelmu);
}

//parital angluar depdent part for Z
TComplex RooDalitzAmplitude::MEangZPart(float helB, int delhelmu,
										float helZ, int helpsi,
										float phiZ, float phiMu, float phiPsi, float alphaMu,
										float thetaB, float thetaPsi) const
{
...
TComplex RooDalitzAmplitude::MEangXPart(float helB, int delhelmu,
                                    float helZ, int helpsi, float help,
                                    float xphiX_X, float xphiMu_X, float xphiPsi_X,
                                    float xalphaMu_X, float thetaB_X, float thetaPsi_X) const
{
  //double partphi = helB*phiZ+helpsi*phiMu+helZ*phiPsi+delhelmu*alphaMu+help*alphaP;
  double partphi = helB*xphiX_X+helpsi*xphiMu_X+(helZ-help)*xphiPsi_X+delhelmu*xalphaMu_X;
  return TComplex(1,partphi,1)*dfunone(thetaPsi_X,helpsi,delhelmu) *
    dfunhalf(thetaB_X,(int)(helB*2.),(int)((helZ-help)*2.));
}
/*
//here convert theta to pi-theta, need to convert couplings
...
//  std::cout << "update coupling " << std::endl;
  ...
  while(RooArgList* Ires = (RooArgList*)coefIter->Next()) {
    ...
    for(int m=0; m<6; ++m) {
       ...
       //do convert here for theta => pi-theta;
       
       float cv = dfun(TMath::Pi()-1.,S,(float)Hel1Two[m]/2.,0.5)/dfun(1.,S,(float)Hel1Two[m]/2.,-0.5)/sin(TMath::Pi()*0.5*Hel1Two[m]);
//       std::cout << "J,m " << S2 << " " <<  Hel1Two[m] << " " << cv << std::endl;
       Hamp[m] = ampCoff*cv/paritypart;
//       std::cout <<"Hamp " << m << Hamp[m]  << std::endl;
    }
    ...
    for(int S1 = abs(S2-2); S1<=S2+2; S1+=2) {
      for(int L1 = S1 - 1; L1<= S1 + 1; L1+=2) {
         ...
         std::cout << "J,S,L " << S2 << " " << S1 <<","<< L1 << " " << M << " " << M1 << " " << M1/M << std::endl;
         ib++;
       }
    }         
    ir++;
...
void RooDalitzAmplitude::CalCoupling() const
{
	...
	while (RooArgList *Ires = (RooArgList *)coefIter->Next())
	{
		...
		m0 = ((RooAbsReal &)ires[iressize - 5]).getVal();
		g0 = ((RooAbsReal &)ires[iressize - 4]).getVal();
		_paraList[ir * nset + MASS] = m0;
		_paraList[ir * nset + WIDTH] = g0;
		//  int parity = (int)((RooAbsReal&)ires[iressize-1]).getVal();
		//  S2 = (int) ((RooAbsReal&)ires[iressize-2]).getVal();
		int parity = _paraList[ir * nset + PARITY];
		S2 = _paraList[ir * nset + SPIN];
		S = (float)S2 / 2.;

		double paritypart;
		//helicity coupling is independent of delmu and Lb helicity
		...
		{ //hel3 proton
			...
			if (hel3 < 0)
				paritypart = pow(-1., (int)(S - 1.5)) * parity;
			// [1/2,1] [1/2,0] [-1/2, 0] [-1/2, -1] [3/2,1] [-3/2, -1]
			for (int m = 0; m < 6; ++m)
			{
				if (fabs(S - 0.5) < 1e-10 && m >= 4)
...
#endif
				ampCoff = ampCoff * TComplex((double)((RooAbsReal &)ires[iressize - 7]).getVal(), (double)((RooAbsReal &)ires[iressize - 6]).getVal());
				amp[ir][k][m] = ampCoff;
				Re = ampCoff.Re();
				Im = ampCoff.Im();
				_vamp.push_back(Re);
		...
		if (_paraList[ir * nset + TYPE] == MODELINDEPENDENT)
		{
			MakeSpline(ires);
			/*
	...
	while (RooArgList *Ires = (RooArgList *)coefIterZ->Next())
	{
		...
		//Rj BW here
		//Pc coupling independent of delmu and Lb helicity
		//    for(int i=0;i<=1; ++i) {//delmu
		//      for(int j=0; j<=1; ++j) { //hel0
		for (int k = 0; k <= 1; ++k)
		{ //hel3 proton
			...
			for (int helpsi = -1; helpsi <= 1; ++helpsi)
			{
				for (int ihz = 0; ihz <= 1; ++ihz)
				{
					...
					{
						help = (ihp == 0) ? -0.5 : 0.5; //helicity of proton in Pc chain
		...
		/*    if(_paraList[ir*nset+TYPE]==MODELINDEPENDENT) {
      for(int isp=0; isp<NUMSP; ++isp) {
        _paraSpline.push_back(((RooAbsReal&)ires[8+(isp)*2]).getVal());
        _paraSpline.push_back(((RooAbsReal&)ires[8+(isp)*2+1]).getVal());
      }
    }     */
		if (_paraList[ir * nset + TYPE] == MODELINDEPENDENT)
		{
		...
		if (_paraList[ir * nset + TYPE] == KMATRIX)
		{
			int ik = 0;
			if ((int)(_paraList[ir * nset + SPIN]) == 3)
				ik = 1;
			if((int)(_paraList[ir*nset+PARITY])>0) 
			        ik += 2;	
			if (_paraList[ir * nset + MASS] > 0)
			{
				for (int ib = 8; ib < 12; ++ib)
					_galpha[ik].push_back(((RooAbsReal &)ires[ib]).getVal());
			}
			else
			{
				_fscat[ik].push_back(((RooAbsReal &)ires[8]).getVal());
			}
		}
		ir++;
		irZ++;
...
//  float help, helX;
        ...
        while(RooArgList* Ires = (RooArgList*)coefIterX->Next()) {
          ...
          m0 = ((RooAbsReal&)ires[iressize-5]).getVal();
          g0 = ((RooAbsReal&)ires[iressize-4]).getVal();
          _paraList[ir*nset] = m0;
          _paraList[ir*nset+1] = g0;
            ...
            for(int helpsi = -1; helpsi <=1; ++helpsi) {
              ...
              for(int ihp = 0; ihp<=1; ++ihp) {
                ...
                for(int ihz = 0; ihz<=1; ++ihz) {
                  ...
                  ampCoff = PartWaveCoffX(ires, S2, (int)(helZ*2.), (int)(helpsi*2.), (int)(help*2.));
#ifdef Misha
                  ...
                  Re = ampCoff.Re();
                  Im = ampCoff.Im();
                  _vamp.push_back(Re); _vamp.push_back(Im);
                  m++;
                }
              }
            }
...
void RooDalitzAmplitude::setup_paraList() const
{
	...
	delete coefIterZ;
        ...
        while(RooArgList* Ires = (RooArgList*)coefIterX->Next()) {
          ...
          //(-1)*(-1)^LR = P_Lz
          if((pow(-1,LR)*parity)<0) LR += 1; //if((pow(-1,LR+1)*parity)<0) LR += 1;
...
void RooDalitzAmplitude::cu_init_data(float *&h_paraList, double *&h_fx, int i_End) const
{
  ...
  for(int i=0;i<array_num; i++) {
    ...
    */
	...
	int size_km = (Npmax + NKMC) * NKMC;
	//  std::cout << "total size " << size_para+size_vamp+size_spline+size_km*2 << std::endl;
	h_paraList = (float *)malloc((size_para + size_vamp + size_spline + size_km * 4) * sizeof(float));
	...
	for (int ik = 0; ik < 4; ++ik)
	{
		//  std::cout << "Km " << ik << " size " << _galpha[ik].size() << " " << _fscat[ik].size() << std::endl;
  ...
  std::cout << "}" << std::endl;*/
	...
	//  std::cout << "shared size " << _paraList.size()+_vamp.size()+_paraSpline.size() << std::endl;
}

Double_t RooDalitzAmplitude::GetAmp2sSMR(Jpsihh_dlz &dlz, Amp_dlz *tnew, bool method1) const
{
	...
	while (ir < NR)
	{
		...
		{
			//need to update
			BW_AMP(m0, g0, mkp, LR, re_i, im_i, 1. / (1. + 0.37));
			break;
		}
			//Bterm = BTerm(S, mkp, m0);
		case REVISEDKMATRIX:
		{
			Revised_KMAMP(mkp, ir, 0, NR, re_i, im_i, 0);
			break;
		}
		case MODELINDEPENDENT:
		{ //for mhh only
			ModelIndependentWave3(&_paraSpline[splcount], mkp, re_i, im_i);
			//a bug here
			//        spline((double)mkp,re_i,im_i);
		...
		//    if(type==MODELINDEPENDENT) std::cout << "ZLM " << mkp << " " << RM.Rho2() << std::endl;
		for (int i = 0; i <= 1; ++i)
		{ //delmu
			//      delhelmu = (i==0) ? -1: 1;
			for (int j = 0; j <= 1; ++j)
			{ //hel0
				...
				{ //hel3 proton
					...
					for (int m = 0; m < 6; ++m)
					{
						...
						ampCoff = amp[ir][k][m];
						tp = ((double)dlz.DL[(S2 - 1) / 2][k][m]) * TComplex((double)dlz.ARe[i][j][m], (double)dlz.AIm[i][j][m]) * RM;
						if (method1)
						{
					...
					for (int helpsi = -1; helpsi <= 1; ++helpsi)
					{
						for (int ihz = 0; ihz <= 1; ++ihz)
						{
							...
							for (int ihp = 0; ihp <= 1; ++ihp)
							{
								...
								tpz = ((double)dlz.DZ[(S2 - 1) / 2][k][m]) * TComplex((double)dlz.AZRe[i][j][m], (double)dlz.AZIm[i][j][m]);
...
//          hel0 = (j==0) ? -0.5: 0.5;
          for(int k=0; k<=1; ++k) { //hel3 proton
//            hel3 = (k==0) ? -0.5: 0.5;
            //there are 12 amplitudes to be added coherently //helpsi,0,+/-1, helX,help+/-1/2, help=+/-1/2
            int m(0);
            for(int helpsi = -1; helpsi <=1; ++helpsi) {
              if(abs(helpsi)>S) continue;
              for(int ihp = 0; ihp<=1; ++ihp) {
                ...
                for(int ihz = 0; ihz<=1; ++ihz) {
                  ...
                  tpz = ((double)dlz.DX[(S2)/2][k][m])
                                  *TComplex((double)dlz.AXRe[i][j][m], (double)dlz.AXIm[i][j][m]) * RM;
	...
	for (int smear_ibin = 0; smear_ibin < _smear_nbins; smear_ibin++)
	{
		...
		while (iPc < NZ)
		{
			...
			case KMATRIX:
			{
				...
				{
//					KMatrix(mjpsip + mjpsip_smear, iS2, -1, _galpha[ik], _fscat[ik], AKM[ik]);
					KMatrix(mjpsip+mjpsip_smear,iS2,((iLR>0)?1:-1),_galpha[ik],_fscat[ik],AKM[ik]);
				}
		...
		while (iPc < NZ)
		{
			...
			while (jPc < NZ)
			{
				...
				//        std::cout << "wmz debug smear BW2PcRR : " << iPc << " " << jPc << "  " << BW2PcRR[iPc][jPc]<< std::endl;
				//        std::cout << "wmz debug smear BW2PcII : " << iPc << " " << jPc << "  " << BW2PcII[iPc][jPc]<< std::endl;
				//        std::cout << "wmz debug smear BW2PcIR : " << iPc << " " << jPc << "  " << BW2PcIR[iPc][jPc]<< std::endl;
				//        std::cout << "wmz debug smear BW2PcRI : " << iPc << " " << jPc << "  " << BW2PcRI[iPc][jPc]<< std::endl;
				jPc++;
			}
			iPc++;
		}
		...
		for (int i = 0; i <= 1; ++i)
		{
			for (int j = 0; j <= 1; ++j)
			{
				...
				while (iPc < NZ)
				{
					...
					//                std::cout << "wmz debug smear L* Z interference: " << totalA << std::endl;
					int jPc = 0;
					while (jPc < NZ)
					{
						totalA += (BW2PcRR[iPc][jPc] + BW2PcII[iPc][jPc]) * (HZ[iPc][i][j][k].Re() * HZ[jPc][i][j][k].Re() + HZ[iPc][i][j][k].Im() * HZ[jPc][i][j][k].Im()) + (BW2PcIR[iPc][jPc] - BW2PcRI[iPc][jPc]) * (HZ[iPc][i][j][k].Re() * HZ[jPc][i][j][k].Im() - HZ[iPc][i][j][k].Im() * HZ[jPc][i][j][k].Re());
						//                        std::cout << "wmz debug smear Z Z interference: " << BW2PcRR[iPc][jPc] << "   " <<  BW2PcII[iPc][jPc] << "  " << HZ[iPc][i][j][k].Re()*HZ[jPc][i][j][k].Re()+HZ[iPc][i][j][k].Im()*HZ[jPc][i][j][k].Im() << "   " << BW2PcIR[iPc][jPc] << "  " <<  BW2PcRI[iPc][jPc] << "    " << HZ[iPc][i][j][k].Re()*HZ[jPc][i][j][k].Im()-HZ[iPc][i][j][k].Im()*HZ[jPc][i][j][k].Re() << std::endl;
						jPc++;
					}
					...
					for (int helpsi = -1; helpsi <= 1; ++helpsi)
					{
						for (int ihz = 0; ihz <= 1; ++ihz)
						{
							...
							for (int ihp = 0; ihp <= 1; ++ihp)
							{
								...
								ampCoff = ampZ[irZ][k][m];
								tpz = ((double)dlz.DZ[(S2 - 1) / 2][k][m]) * TComplex((double)dlz.AZRe[i][j][m], (double)dlz.AZIm[i][j][m]) * RM;
								if (method1)
								{
									tnew->AZRe[irZ][i][j][k][m] = tpz.Re();
									tnew->AZIm[irZ][i][j][k][m] = tpz.Im();
...
Double_t RooDalitzAmplitude::GetAmp2sForInt_SMR(Jpsihh_dlz &dlz, int index, int group, Amp_dlz *tnew, bool method1) const
{
//  TComplex H[2][2][2];//delhelmu=+/-1,hel0=+/-1/2,hel3=+/-1/2
...
//  std::cout << "wmz debug start Lst" << std::endl;
  while( ir<NR ) {
    ...
    LR = (int)(_paraList[ir*nset+LRESON]);
    type = (int) (_paraList[ir*nset+TYPE]);
    if((group!=1)&&!(group==0&&ir==index)&&(group!=3&&group!=4)) {ir++; if(type==MODELINDEPENDENT) splcount += 9*NUMSP;; continue;}
    if( (group==3) && ((LR+S2)!=index) ) {ir++; if(type==MODELINDEPENDENT) splcount += 9*NUMSP;; continue;}
      ...
      for(int j=0; j<=1; ++j) { //hel0
        ...
        for(int k=0; k<=1; ++k) { //hel3 proton
          ...
          for(int m=0; m<6; ++m) {
            ...
            if(fabs(S-0.5)<1e-10&&m>=4) {
             if(method1) { tnew->ARe[ir][i][j][k][m] = 0.0;
                         tnew->AIm[ir][i][j][k][m] = 0.0;
              }
              continue;
            }
            ...
            ampCoff = amp[ir][k][m];
            tp = ((double)dlz.DL[(S2-1)/2][k][m]) * TComplex((double)dlz.ARe[i][j][m],(double)dlz.AIm[i][j][m]) * RM;
            if(method1){ tnew->ARe[ir][i][j][k][m] = tp.Re();
                          tnew->AIm[ir][i][j][k][m] = tp.Im();
             }
...
//  std::cout << "wmz debug start Pc" << std::endl;
  while ( irZ<NZ )  {
    ...
    type = (int) (_paraList[ir*nset+TYPE]);
    if((group!=2)&&!(group==0&&ir==index)&&(group!=5)) {ir++; irZ++; continue;}
    if( (group==5) && ((LR+2*S2)!=index) ) {ir++;irZ++; continue;}
          ...
          for(int helpsi = -1; helpsi <=1; ++helpsi) {
            for(int ihz = 0; ihz<=1; ++ihz) {
              ...
              for(int ihp = 0; ihp<=1; ++ihp) {
                help = (ihp==0) ? -0.5: 0.5;
                if(fabs((float)helpsi-help)>S) {
                 if(method1) {tnew->AZRe[irZ][i][j][k][m] = 0.0;
                             tnew->AZIm[irZ][i][j][k][m] = 0.0;         }
                        m++; continue;}
                ampCoff = ampZ[irZ][k][m];
                tpz = ((double)dlz.DZ[(S2-1)/2][k][m])
                                  *TComplex((double)dlz.AZRe[i][j][m], (double)dlz.AZIm[i][j][m]);
                                  //*TComplex(dlz.AZRe[i][j][m], dlz.AZIm[i][j][m]) * RM;
                if(method1){ tnew->AZRe[irZ][i][j][k][m] = tpz.Re();
                           tnew->AZIm[irZ][i][j][k][m] = tpz.Im();                  }
                if((ampCoff.Rho2())<1e-10) { m++; continue;     }
...
//  std::cout << "wmz debug start Zcs" << std::endl;
  while ( irX<NX )  {
    if((group!=10)&&!(group==0&&ir==index)) {ir++; irX++; continue;} //10 for Zcs contribution
    S2 = (int) (_paraList[ir*nset+SPIN]);
    S = (float)S2/2.;
...
//       int isp(0);
       if((group!=2)&&!(group==0&&iir==index)&&(group!=5)) {
               iPc++; 
               if (type == MODELINDEPENDENT)  splcount += 9 * NUMSPZ;
               if (type == KMATRIX) {
                 int ik(0); 
                 if (iS2 == 3)
                         ik = 1;
...
/*       case MODELINDEPENDENT:
        {
         ModelIndependentWave(&_paraSpline[isp*NUMSP*2], mjpsip+mjpsip_smear,re_i, im_i);
         //Bterm = BTermZ(S, mjpsip,-1000);        
         im0 = -1000;
         isp ++;
         break;
...
//                 std::cout << "wmz debug AKM before: " << AKM[ik][ikmcount[ik]]<< "   " << mjpsip << "   " << mjpsip_smear << "   " <<  ikmcount[ik]<<  "   "  << iPc <<std::endl;
                 if (AKM[ik][0].Rho2() < 1e-10)
                 {
//                       KMatrix(mjpsip + mjpsip_smear, iS2, -1, _galpha[ik], _fscat[ik], AKM[ik]);
                         KMatrix(mjpsip+mjpsip_smear,iS2,((iLR>0)?1:-1),_galpha[ik],_fscat[ik],AKM[ik]);
                 ...
                 re_i = AKM[ik][ikmcount[ik]].Re();
                 im_i = AKM[ik][ikmcount[ik]].Im();
//                 std::cout << "wmz debug AKM: " << AKM[ik][ikmcount[ik]]<< "   " << mjpsip << "   " << mjpsip_smear << std::endl;
                 ikmcount[ik]++;
...
//       RM = TComplex(re_i,im_i)*Bterm;
        BWPc_tmp[iPc] = TComplex((double)re_i*iBterm, (double)im_i*iBterm, false);
       BW1Pc[iPc] += TComplex((double)re_i*iBterm, (double)im_i*iBterm, false);
//       std::cout << "wmz debug BW1Pc: " << re_i << "  " << im_i << "   " << BTermZ(iS, mjpsip+mjpsip_smear, im0) << "  " << weight_smear << std::endl;
       iPc++;
    }
...
//        std::cout << "wmz debug smear L* part: " << totalA << std::endl;
        ...
        while (iPc < NZ){
                ...
                Amp1L = H_L_Zcs[i][j][k];
                totalA += 2*(Amp1Pc.Re() * Amp1L.Re() + Amp1Pc.Im() * Amp1L.Im());
//                std::cout << "wmz debug smear L* Z interference: " << Amp1Pc.Re() << " " << Amp1L.Re() << "  " << Amp1Pc.Im() << "  " << Amp1L.Im() << "  " << HZ[iPc][i][j][k] << "   " << H_L_Zcs[i][j][k] << std::endl;
                int jPc = 0;
                while (jPc < NZ){
                    totalA += (BW2PcRR[iPc][jPc] + BW2PcII[iPc][jPc]) * (HZ[iPc][i][j][k].Re()*HZ[jPc][i][j][k].Re()+HZ[iPc][i][j][k].Im()*HZ[jPc][i][j][k].Im())
                          +  (BW2PcIR[iPc][jPc] - BW2PcRI[iPc][jPc]) * (HZ[iPc][i][j][k].Re()*HZ[jPc][i][j][k].Im()-HZ[iPc][i][j][k].Im()*HZ[jPc][i][j][k].Re());
//                        std::cout << "wmz debug smear Z Z interference: " << BW2PcRR[iPc][jPc] << "   " <<  BW2PcII[iPc][jPc] << "  " << HZ[iPc][i][j][k].Re()*HZ[jPc][i][j][k].Re()+HZ[iPc][i][j][k].Im()*HZ[jPc][i][j][k].Im() << "   " << BW2PcIR[iPc][jPc] << "  " <<  BW2PcRI[iPc][jPc] << "    " << HZ[iPc][i][j][k].Re()*HZ[jPc][i][j][k].Im()-HZ[iPc][i][j][k].Im()*HZ[jPc][i][j][k].Re() << std::endl;
              jPc++;
            }
...
Double_t RooDalitzAmplitude::GetAmp2sForInt(Jpsihh_dlz &dlz, int index, int group) const
{
			...
			{ //hel0
				...
				{ //hel3 proton
					...
					for (int m = 0; m < 6; ++m)
					{
						...
						if ((ampCoff.Rho2()) < 1e-10)
							continue;
						H[i][j][k] += ampCoff * TComplex((double)dlz.ARe[i][j][m], (double)dlz.AIm[i][j][m]) * RM * ((double)dlz.DL[(S2 - 1) / 2][k][m]);
					}
				}
			}
		}
	...
	while (RooArgList *Ires = (RooArgList *)coefIterZ->Next())
	{
		...
		if (S2 == 3)
			ik = 1;
                if(LR>0) {
                    if(parity<0) std::cout << "LR not match parity" << std::endl;
                    ik += 2;
                }
					...
					for (int helpsi = -1; helpsi <= 1; ++helpsi)
					{
						for (int ihz = 0; ihz <= 1; ++ihz)
						{
							...
							for (int ihp = 0; ihp <= 1; ++ihp)
							{
								...
								H[i][j][k] += ampCoff * TComplex((double)dlz.AZRe[i][j][m], (double)dlz.AZIm[i][j][m]) * RM * ((double)dlz.DZ[((S2 - 1) / 2)][k][m]); //AIndep;
								m++;
							}
						}
					}
...
//                                  *TComplex((double)dlz.AXRe[i][j][m], (double)dlz.AXIm[i][j][m]) * RM;
                  ...
                  H[i][j][k] += ampCoff * ((double)dlz.DX[(S2)/2][k][m])*TComplex((double)dlz.AXRe[i][j][m], (double)dlz.AXIm[i][j][m]) * RM;
                  m++;
                }
              }
            }
...
TComplex RooDalitzAmplitude::PartWaveCoffZ(RooArgList &ires, int J, int lambdaZ, int lambdaPsi, int lambdap) const
{
	...
	for (int S = 1; S <= 3; S += 2)
	{
		for (int L = abs(J - S); L <= J + S; L += 2)
		{
			...
			//      if(m1<0&&L>S) tmp   = -tmp;
			//std::cout << "Test CG spin L S m1 " << J << " " << L << " " << S << " " << m1<< " " << c1.coef(S,m1,s2,s3,lambda2,-lambda3)*c2.coef(JB,m1,L,S,0,m1) << std::endl;
			//Misha's two particle LS coupling ===
...
TComplex RooDalitzAmplitude::PartWaveCoffX(RooArgList ires, int J, int lambdaZ, int lambdaPsi, int lambdap) const
{
  int ib(4);
  //Coupling for Zc
  TComplex HZ(0,0);
  ...
  for(int LZ = J - SZ; LZ<= SZ + J; LZ+=2) {
    if(J==0&&LZ!=SZ) continue;
    if(parity*pow(-1,(int)(LZ/2))<0) continue;
    ...
    if(first) {
      ...
      TComplex M(((RooAbsReal&)ires[2*ib]).getVal(),((RooAbsReal&)ires[2*ib+1]).getVal());
  ...
  for(int S = J-1; S <=J+1 ; S+=2) {
    ...
    for(int L = S-1; L<= S+1; L+=2) {
      //(-1)*(-1)^LR = P_Lz
      //std::cout << "S L " << S << " " << L << std::endl;
      if(S<abs(m1)) std::cout << "why lzhang S<m1" << std::endl;
      ccg1 = C2[(S-1)/2][(L-S+1)/2];
      if(m1<0&&L>S) ccg1   = -ccg1;
      //  EvtCGCoefSingle c1(s2,s3);
      //std::cout << "ib " << ib << std::endl;
      if(ib>=4) std::cout << "why X parity" << std::endl;
      TComplex M(((RooAbsReal&)ires[ib*2]).getVal(),((RooAbsReal&)ires[ib*2+1]).getVal());
      //std::cout << "M " << M << std::endl;
      if((M.Rho2()<1e-10)) { ib++; continue;}
...
Float_t RooDalitzAmplitude::BTerm(Float_t Spin, Float_t mpp, Float_t mr) const
{
	...
	if (mr + m_jpsi > m_lb)
	{
		float mmin = m_k + m_p;
		float mmax = m_lb - m_jpsi;
		float mreff = mmin + 0.5 * (mmax - mmin) * (1.0 + tanh((mr - (mmin + mmax) / 2.) / (mmax - mmin)));
		pB0 = getp(m_lb, m_jpsi, mreff);
	}
	//  float phi00 = ((RooAbsReal&)ires[ihel*2+1]).getVal()*deg2rad;
	//  c = F_BW(_FFB,LB,pB,pB0) * pow(pB/pB0, LB);
#ifdef Lst_LBPlusOne
        c = F_BW(_FFB, LB+1, pB, pB0) * pow(pB / m_lb, LB) * pow(m_lb / pB0, LB) * (pB/pB0);
#else
	c = F_BW(_FFB, LB, pB, pB0) * pow(pB / m_lb, LB) * pow(m_lb / pB0, LB);
#endif
...
Float_t RooDalitzAmplitude::BTermZ(Float_t Spin, Float_t mpp, Float_t mr) const
{
	...
	//  float phi00 = ((RooAbsReal&)ires[ihel*2+1]).getVal()*deg2rad;
#ifdef Pc_LBPlusOne
	c = F_BW(_FFB, LB+1, pB, pB0) * pow(pB / m_lb, LB) * (pB/pB0);
#else
...
////////////////////////////////////////////////
void RooDalitzAmplitude::breit_wigner(float m, float gamma, float e2, float &r_ampl, float &i_ampl) const
{
	float de2 = m * m - e2;
...
Float_t RooDalitzAmplitude::F_BW(float r, int L, float p, float p0) const
{
	...
	if (L == 3)
		return sqrt((z0 * z0 * z0 + 6 * z0 * z0 + 45 * z0 + 225) / (z * z * z + 6 * z * z + 45 * z + 225));
	if (L == 4)
		return sqrt((pow(z0, 4) + 10 * pow(z0, 3) + 135 * z0 * z0 + 1575 * z0 + 11025) / (pow(z, 4) + 10 * pow(z, 3) + 135 * z * z + 1575 * z + 11025));
	if (L == 5)
		return sqrt((pow(z0, 5) + 15 * pow(z0, 4) + 315 * pow(z0, 3) + 6300 * pow(z0, 2) + 99225 * z0 + 893025) / (pow(z, 5) + 15 * pow(z, 4) + 315 * pow(z, 3) + 6300 * pow(z, 2) + 99225 * z + 893025));
...
Float_t RooDalitzAmplitude::F_BW2(float r, int L, float p2, float p20) const
{
  ...
  if(L==2) return ((z0*z0+3*z0+9.)/(z*z+3*z+9.));
  if(L==3) return ((z0*z0*z0+6*z0*z0+45*z0+225)/(z*z*z+6*z*z+45*z+225));
  if(L==4) return ((pow(z0,4)+10*pow(z0,3)+135*z0*z0+1575*z0+11025)
                         /(pow(z,4)+10*pow(z,3)+135*z*z+1575*z+11025));
  if(L==5) return ((pow(z0,5)+15*pow(z0,4)+315*pow(z0,3)+6300*pow(z0,2)+99225*z0+893025)
...
cnum RooDalitzAmplitude::get_b(cnum alpha, cnum p23) const
{
	...
	cnum value = unit + unit / p23 / p23 * (alpha * (p12 * p12 - p13 * p13 - p23 * p23) + m3 * m3 - m2 * m2) - 0.001 * ii;
...
cnum RooDalitzAmplitude::get_c(cnum alpha, cnum p23, float m_r, float gamma) const
{
	...
	cnum value = unit / p23 / p23 * (m3 * m3 + alpha * (m1 * m1 - m3 * m3) - alpha * (unit - alpha) * p13 * p13);
	return value;
}
...
cnum RooDalitzAmplitude::this_func(float alpha1, float p23_real, float m_r, float gamma) const
{
	...
	cnum alpha(alpha1, 0);
	cnum p23(p23_real, 0);
	cnum sqrt_value = sqrt(unit4 * get_c(alpha, p23, m_r, gamma) - get_b(alpha, p23) * get_b(alpha, p23));
	cnum value = unit / sqrt_value * (atan(get_b(alpha, p23) / sqrt_value) - atan((get_b(alpha, p23) + unit2 * (alpha - unit)) / sqrt_value));

	return value;
}

void RooDalitzAmplitude::get_cusp_ampli(float m_r, float gamma, float p23, float &r_ampl, float &i_ampl) const
{
...
// CUSP Amplitude
/////////////////////////////////////////////////
bool RooDalitzAmplitude::GetCusp_AMP(float m_r, float gamma_r, float mpp,
									 float &ampl_r, float &ampl_i) const
{
	ampl_r = 1;
	ampl_i = 0;
...
/////////////////////////////////////////////////
bool RooDalitzAmplitude::BW_AMP(float m_r, float gamma_r, float mpp, int LR,
								float &ampl_r, float &ampl_i) const
{
	ampl_r = 0;
	ampl_i = 0;
	...
	if (m_r <= m_p + m_k && m_r > 1e-10)
	{
		float mmin = m_k + m_p;
		float mmax = m_lb - m_jpsi;
		float mreff = mmin + 0.5 * (mmax - mmin) * (1.0 + tanh((m_r - (mmin + mmax) / 2.) / (mmax - mmin)));
		pR0 = getp(mreff, m_p, m_k);
	}
...
//For Flatte
bool RooDalitzAmplitude::BW_AMP(float m_r, float gamma_r, float mpp, int LR,
								float &ampl_r, float &ampl_i, float br1) const
{
	ampl_r = 0;
	ampl_i = 0;
	float m2pp = mpp * mpp;
...
bool RooDalitzAmplitude::BW_AMPZ(float m_r, float gamma_r, float mpp, int LR,
								 float &ampl_r, float &ampl_i) const
{
	ampl_r = 0;
	ampl_i = 0;
...
bool RooDalitzAmplitude::BW_AMPZSMR(float m_r, float gamma_r, float mpp, int LR,
									float &ampl_r, float &ampl_i) const //out-of-date not used
{
...
bool RooDalitzAmplitude::BW_AMPX(float m_r, float gamma_r, float mpp, int LR,
                             float &ampl_r, float &ampl_i) const
{
  ampl_r = 0;
  ampl_i = 0;
...
bool RooDalitzAmplitude::KMatrix(float mpp, int J_r, int P_r, std::vector<float> & g_alpha, std::vector<float> &f_scat, std::vector<TComplex> &Mtr) const//only for Pc 
{
  ...
  for(int ir=NR; ir<NR+NZ; ++ir) {
    ...
    if(type==KMATRIX&&S==J_r&&parity==P_r) {
      ...
      if(m0>0) {
	denorm = (m0*m0-s);
	if(fabs(denorm)<1e-3) denorm=1e-3;
	Npole++;
	ms[0] *= denorm;
	for(int ip=1; ip<=Npmax; ++ip) {
	  if(ip==ikm) continue;
...
//  std::cout << "g_alpha[0], f_scat[0] " << g_alpha[0] << " " << f_scat[0] << std::endl;
  ...
  TComplex rho[NKMC];//do calculation here======needed
  if(P_r==-1) {
    for(int i=0; i<NKMC-1; ++i)   rho[i] = getrho(s,Km1[i],Km2[i]);
    if(1==J_r) {
      rho[NKMC-1] = getrho(s,Km1[3],Km2[3]);
    //std::cout << "one Channel " << Km1[3] << "--" << Km2[3] << std::endl;
    }else if(3==J_r) {
  ...
  int ifail;
  SMatrixNN IIB = IB.Inverse(ifail);
  if(ifail) std::cout<<"inverse matrix failed 1"<<std::endl;           
  SMatrixNN RR = (IB+A*IIB*A);
  SMatrixNN IRR = RR.Inverse(ifail);
  if(ifail) {
    std::cout<<"inverse matrix failed 2 mjpsip "<< sqrt(s) << std::endl;  
...
//first raw for Jpsi p    
  ...
  float na = 1.0f;
  if(Lna>=1) na = getFBW2(s, Km1[0], Km2[0],Lna);
  for(int ip=0; ip<Npole; ++ip) {
    for(int i=0; i<NKMC; ++i) {
      Mtr[ip] +=T[i]*double(mg[ip*NKMC+i]);
    }
...
bool RooDalitzAmplitude::Revised_KMAMP(float mpp, int nr, int irst, int ired, float &ampl_r, float &ampl_i, bool isPcChain) const
{
	float m2pp = mpp * mpp;
	float m1, m2, m3;
	...
	else if (m_r >= 0)
	{
		...
		mreff = mmin + 0.5 * (mmax - mmin) * (1.0 + tanh((m_r - (mmin + mmax) / 2.) / (mmax - mmin)));
	}
	else
	{ //NR
	...
	if (m_r > 0)
	{
		...
		{ //Only validate for 1405 !!
			...
			denorm = denorm - fsc * 0.0505 * pow((pRSigPi / pR0), 2 * L_r + 1) * (m_r / mpp) * FR * FR * m_r;
		}
		totG += m_r * gamma;
	...
	{																			//For NR
																				//    fsc = gamma_r;
		totG += gamma_r * pow((pR / pR0), 2 * L_r + 1) * (m_r / mpp) * FR * FR; //KMatrixTerm(m_r, gamma_r, mpp, L_r, fsc);
...
float RooDalitzAmplitude::KMatrixTerm(float m_r, float gamma_r, float mpp, int LR, float &fsc) const
{
...
void RooDalitzAmplitude::filldlz(Jpsihh_dlz &dlz, Double_t xmkp, Double_t xcostheta,
                                                                 ...
                                                                 Double_t xcosthetap, Double_t xalphaMu,
                                                                 Double_t xmjpsip,
                                                                 Double_t xcosthetaB_X, Double_t xcosthetaX_X, Double_t xcosthetaPsi_X,
                                                                 Double_t xphiX_X, Double_t xphiPsi_X, Double_t xphiMu_X,
                                                                 Double_t xcosthetap_X, Double_t xalphaMu_X,
                                                                 Double_t xmjpsik,
                                                                 Double_t sw) const
{
	dlz.mkp = xmkp;
			...
			for (int helpsi = -1; helpsi <= 1; ++helpsi)
			{
				for (int ihz = 0; ihz <= 1; ++ihz)
				{
					...
					for (int ihp = 0; ihp <= 1; ++ihp)
					{
						...
						AIndep = MEangZPart(hel0, delhelmu, helZ, helpsi, xphiZ, xphiMu, xphiPsi, xalphaMu, thetaB, thetaPsi);
			...
			for (int helpsi = -1; helpsi <= 1; ++helpsi)
			{
				for (int ihz = 0; ihz <= 1; ++ihz)
				{
					...
					for (int ihp = 0; ihp <= 1; ++ihp)
					{
						...
						dlz.DZ[s][k][m] = dfunhalf(thetap, (int)(help * 2.), (int)(hel3 * 2.)) * //hel3 = hel_p in /\*
...
//              if(abs(helpsi)>S) continue; 
              for(int ihp = 0; ihp<=1; ++ihp) { // In Lb rest frame
                ...
                for(int ihz = 0; ihz<=1; ++ihz) {
                  helZ = (ihz==0) ? help-0.5: help+0.5;
//                  if(fabs(helZ)>S) continue;
                  AIndep =  MEangXPart(hel0,delhelmu, helZ, helpsi, help, xphiX_X, xphiMu_X, xphiPsi_X, xalphaMu_X, thetaB_X, thetaPsi_X);//hel0,delhelmu, helZ, helpsi, xphiZ, xphiMu, xphiPsi, xalphaMu, thetaB, thetaPsi);
              ...
              for(int helpsi = -1; helpsi <=1; ++helpsi) {
                ...
                for(int ihp = 0; ihp<=1; ++ihp) {
                  ...
                  for(int ihz = 0; ihz<=1; ++ihz) {
                    helZ = (ihz==0) ? help-0.5: help+0.5;
                    if(fabs(helZ)>S) continue;
//                    if(fabs((float)helpsi-help)>S) { dlz.DZ[s][k][m]=0;  m++; continue; }
                    dlz.DX[s][k][m]= dfunhalf(thetap_X,(int)(help*2.),(int)(hel3*2.)) * //hel3 = hel_p in /\*
