...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl; 
  ...
  TString PHSPcut = TString(argv[10]);
  TString mcswName = TString(argv[11])+".root";
  std::cout << PHSPcut << std::endl;
  std::cout << mcswName << std::endl;
...
//     if(i%1000==0) std::cout << "i " << i << std::endl;
  ...
  RooDataSet *datars2 = (RooDataSet*)datars->reduce(PHSPcut);
...
//     if(i%1000==0) std::cout << "i " << i << std::endl;
  ...
  datars2->merge(IND2); 
  RooDataSet * data_fitInRegion = new  RooDataSet(TString(datars2->GetName())+TString("newInRegion"),datars2->GetTitle(),datars2,*datars2->get(),0,"nsig_sw");
...
//  std::cout << "will write to file: " << TString::Format("func/testKM4Pc-fullPHSP-fineBinning-ExtLst-4440%s-4457%s-4312%s-4600%s-"+label_for_output+".func",nameZ5, name, nameZ4,nameZ2).Data() << std::endl;
//
//
//  TList* fixedPcpars = fixPar_getfixedPar(listZ);
//
//  if(!(ZS4==3&&ZP4==-1)) {
//    rg4_Z4.setConstant(1);
//  }
//  rg4_Z4.setConstant(1);
...
//  setdlz->writeToFile(TString::Format("func/testKM4Pc-fullPHSP-fineBinning-ExtLst-4440%s-4457%s-4312%s-4600%s-"+label_for_output+".func",nameZ5, name, nameZ4,nameZ2).Data());
//  setdlz->Print("V");
//  std::cout << "NLLHalfwFloat " << NLL2 << std::endl;    
   double llInRegion = LL(obs, list, listZ, listX,  data_fitInRegion,  mcswName, FFR_var, FFB_var, errlist);
   std::cout << llInRegion << std::endl;
   printf("LLCheck-all-InRegion: %10.3f,  %10.3f\n",NLL2,llInRegion);
  return 0;
...
//    signew->genToy("mcsw.root",label_for_output, true, true, true, false);
