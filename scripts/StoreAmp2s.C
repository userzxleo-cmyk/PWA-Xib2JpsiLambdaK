...
auto parse_args(int argc, char **argv){
  ...
  options
    ...
    ("i,input", "Path to input nTuple", cxxopts::value<TString>())
    ("o,output", "Path to output nTuple", cxxopts::value<TString>())
    ...
    std::cout << "Reading model from roofitresult : " + args["result"].as<TString>() << std::endl;
    std::cout << std::endl;

    std::cout << "Reading input from : " + args["input"].as<TString>() << std::endl;
    std::cout << std::endl;

    std::cout << "Will write per-event amp2s to : " + args["output"].as<TString>() << std::endl;
    std::cout << std::endl;
...
//  cosk:cosmu:chi
  ...
  obs1->add(sw);
  TFile *fdata = new TFile(args["input"].as<TString>());
  TTree *datree = (TTree*)fdata->Get("tree");
...
//  list->Add(L_2000);  
  ...
  std::cout << "Calculating Amp^2 for file " << args["input"].as<TString>() << std::endl;

  sigcopy->storeAmp2s(args["output"].as<TString>());
