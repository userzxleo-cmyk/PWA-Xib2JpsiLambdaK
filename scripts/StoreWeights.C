...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl; 
  ...
  TString mc_input_path = TString(argv[13]);
  TString dataPath = TString(argv[14]);
  TString mcPath = TString(argv[15]);
...
//  list->Add(L_2000);  
  ...
  RooDalitzAmplitude *sig = new RooDalitzAmplitude("sig","", *obs, list, listZ, listX, *FFR_var, *FFB_var,  "mcsw.root", *data_fit, *widthUpperLimit_var, *widthLowerLimit_var, *CalFWHM_nbin_var, *CalFWHM_range_var);  
...
//  width_1520.setConstant(0);
  ...
  std::cout << "will read from file file: " << mc_input_path << std::endl;
...
//  m0_Z4.setError(0.0001);
  ...
  setdlz->readFromFile(name_initial_setting);
  RooDalitzAmplitude *signew = new RooDalitzAmplitude("signew","", *obs, list, listZ, listX, *FFR_var, *FFB_var, mc_input_path, *data_fit, *widthUpperLimit_var, *widthLowerLimit_var, *CalFWHM_nbin_var, *CalFWHM_range_var);
  RooDalitzAmplitude *sigcopy = new RooDalitzAmplitude(*signew, "storeweights");
  std::cout << "Calculating Amp^2 weights for file " << mc_input_path << std::endl;

  sigcopy->storeGPUWeights(dataPath, mcPath);
