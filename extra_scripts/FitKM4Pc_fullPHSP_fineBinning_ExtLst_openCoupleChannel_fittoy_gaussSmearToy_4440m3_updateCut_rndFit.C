...
bool kinelimit(double mkp, double mjpsip)
{
  double      m_lb = 5.61951;
  double      m_k = 0.493677;  //1
  double      m_p = 0.938272046; //2
  double      m_jpsi = 3.096916; //3
  
  double m2_lb = m_lb*m_lb;
  double m2_k = m_k*m_k;
  double m2_p = m_p*m_p;
  double m2_jpsi = m_jpsi*m_jpsi;
  double m2kp = mkp*mkp;
  double m2jpsip = mjpsip*mjpsip;
  double mjpsik;
  double p21,p22,p23;
  double e2_star, e3_star;
  double p2_star, p3_star, e_sum2;
  double m2_23_min, m2_23_max;
  //if(!(mkp>m_k+m_p&&mkp<m_lb-m_jpsi)) return false;
 // if(!(mjpsip>m_jpsi+m_p&&mjpsip<m_lb-m_k)) return false;  
  mjpsik = m2_lb+m2_k+m2_p+m2_jpsi-m2kp-m2jpsip;
  mjpsik = sqrt(mjpsik);
//  if(!(mjpsik>m_jpsi+m_k&&mjpsik<m_lb-m_p)) return false;
  p21 = 1./4./m2_lb*(m2_lb-pow(mjpsip+m_k,2))*(m2_lb-pow(mjpsip-m_k,2));
  p22 = 1./4./m2_lb*(m2_lb-pow(mjpsik+m_p,2))*(m2_lb-pow(mjpsik-m_p,2));
  p23 = 1./4./m2_lb*(m2_lb-pow(mkp+m_jpsi,2))*(m2_lb-pow(mkp-m_jpsi,2));
  if(!(p21>=0&&p22>=0&&p23>=0)) return false;
  e2_star = (m2kp-m2_k+m2_p)/2./mkp;
  e3_star = (m2_lb-m2kp-m2_jpsi)/2./mkp;
  if(e2_star<m_p || e3_star<m_jpsi) return false;
  p2_star = sqrt(pow(e2_star,2)-m2_p);
  p3_star = sqrt(pow(e3_star,2)-m2_jpsi);
  e_sum2 = pow(e2_star+e3_star,2);
  m2_23_max = e_sum2 - pow(p2_star-p3_star,2);
  m2_23_min = e_sum2 - pow(p2_star+p3_star,2);
  if(m2jpsip<m2_23_min || m2jpsip>m2_23_max) return false;
...
int main(int argc, char **argv)
{
 ...
 std::cout << "Time(begin)  " << BeginTime.GetYear() << "." << BeginTime.GetMonth() << "." << BeginTime.GetDay() << "    " << BeginTime.GetHour() << ":" << BeginTime.GetMinute() << ":" << BeginTime.GetSecond() << std::endl; 
  ...
  TString name_toyfile = TString(argv[10]);
//  TString output_func_name = TString(argv[11]);
  //name_toyfile = "../gen_toys/re_arrange_mjpsip/output/"+name_toyfile;
  //name_toyfile = "../toy_tuples/"+name_toyfile;
  name_toyfile = "../gen_toys/re_arrange_mjpsip/output/"+name_toyfile;

  int seed = atoi( argv[11] );
  g_rnd = new TRandom2();
  g_rnd->SetSeed(seed);
  TString output_func_name = "func/"+name_initial_setting+"_rndFit_seed"+TString::Format("%d", seed)+".func";
  name_initial_setting = "func/"+name_initial_setting+".func";
//  output_func_name = "func/" + output_func_name+".func";
  std::cout << "name of initial config file is: " << name_initial_setting << std::endl;
  std::cout << "name of fit toy file: " << name_toyfile << std::endl;
  std::cout << "name of output func: " << output_func_name << std::endl;


  TString fitresultName = TString(argv[12]);
...
//  TFile *fdata = new TFile("sPlot.root");
  TFile *fdata = new TFile(name_toyfile);
  TTree *datree = (TTree*)fdata->Get("DTree");
  

  TString tfname = TString(argv[12]);
  TFile tf1(TString("temp/") + tfname + ".root","recreate");
  cout<<datree->GetEntries()<<endl;
  auto newdata = datree->CopyTree("mkp>1.431950001&&mkp<2.522583999999&&mjpsip>4.0351880460&&mjpsip<5.125823");
  cout<<newdata->GetEntries()<<endl;
  double tmkp,tmjpsip,out;
  newdata->SetBranchAddress("mkp",&tmkp);
  newdata->SetBranchAddress("mjpsip",&tmjpsip);
  auto brout = newdata->Branch("out",&out,"out/D");
  int nit = newdata->GetEntries();
  for(int i = 0;i < nit;i++)
  {
      newdata->GetEntry(i);
      if(kinelimit(tmkp,tmjpsip))
          out = 1;
      else
          out = 0;
      brout->Fill();
  }
  auto finaldata = newdata->CopyTree("out>0.5");
  cout<<finaldata->GetEntries()<<endl;
  RooDataSet* datars1 = new RooDataSet("datars","",finaldata, *obs2);
...
//  list->Add(L_2000);  
  ...
  RooDalitzAmplitude *sig = new RooDalitzAmplitude("sig","", *obs, list, listZ, listX, *FFR_var, *FFB_var,  "one.root",*data_fit);  
...
//  width_1520.setConstant(0);
  ...
  std::cout << "will write to file: " << output_func_name << std::endl;


//  NLL2 = doFit(obs, list, listZ, listX,  data_fit, fitresultName, FFR_var, FFB_var, 0, true, errlist, false, false);
  //NLL2 = doFit(obs, list, listZ, listX,  data_fit, fitresultName, FFR_var, FFB_var, 0, true, errlist, false, false);
  NLL2 = doFit(obs, list, listZ, listX,  data_fit, fitresultName, FFR_var, FFB_var, 0, true, errlist, true, false);

  setdlz->writeToFile(output_func_name);
  setdlz->Print("V");
  ...
  return 0;
  if(0){
  RooDalitzAmplitude *signew = new RooDalitzAmplitude("signew","", *obs, list, listZ, listX, *FFR_var, *FFB_var, "mcsw.root",*data_fit);
//  setdlznew = setdlz;
...
//  signew->genToy("Toy");        
  ...
  if(1) {
  RooDalitzAmplitude *sigflat = new RooDalitzAmplitude("sigflat","", *obs, list, listZ, listX, *FFR_var, *FFB_var, "MCFlatAcc.root", *data_fit);
